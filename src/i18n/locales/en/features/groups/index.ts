export const groupsTranslations = {
  title: 'Groups',
  description: 'Manage and participate in groups',
  headerDescription: 'Discover and join communities working on important causes',
  hierarchicalMembershipDisabled: 'Members join hierarchical groups through subgroups',
  automaticSiblingMembershipDisabled: 'This sibling group uses automatic memberships',
  openSiblingMembershipRequiresConnectedGroupMember:
    'Only active members of the connected group can join this sibling group',
  create: 'Create Group',
  edit: 'Edit Group',
  delete: 'Delete Group',
  join: 'Join Group',
  leave: 'Leave Group',
  backToGroups: 'Back to Groups',
  editPage: {
    title: 'Edit Group',
    subtitle: 'Update group information',
    loading: 'Loading group data...',
    notFound: 'Group not found',
    notFoundDescription: 'No group data exists for this ID',
    saveChanges: 'Save Changes',
    saving: 'Saving...',
    groupImageAlt: 'Group image',
    groupType: {
      title: 'Group Type',
      description: 'This section shows how the group is currently structured.',
      label: 'Current structure',
      hierarchicalDescription: 'This group is composed of subordinate groups.',
      baseDescription: 'This group is a base group with direct members.',
      mixedDescription:
        'This group has active subgroups and is also connected to at least one sibling group.',
    },
  },
  location: {
    title: 'Location',
    description: 'Where is this group based?',
    countryPlaceholder: 'Country',
    regionPlaceholder: 'State, province, or region',
    postCodePlaceholder: 'Post code',
    cityPlaceholder: 'City',
    streetPlaceholder: 'Street',
    houseNumberPlaceholder: 'House number',
  },
  contact: {
    title: 'Contact & Links',
    description: 'Share the best ways to discover and reach this group online.',
    socialTitle: 'Social profiles',
    socialDescription: 'Add the channels where members and visitors can follow updates.',
    emailLabel: 'Email',
    emailPlaceholder: 'group@example.org',
    websiteLabel: 'Website',
    websitePlaceholder: 'https://yourgroup.org',
    youtubeLabel: 'YouTube',
    youtubePlaceholder: '@groupchannel or channel URL',
    linkedinLabel: 'LinkedIn',
    linkedinPlaceholder: 'Profile URL or /in/handle',
    whatsappLabel: 'WhatsApp',
    whatsappPlaceholder: 'Phone number or WhatsApp link',
    instagramLabel: 'Instagram',
    instagramPlaceholder: '@groupname or profile URL',
    twitterLabel: 'Twitter/X',
    twitterPlaceholder: '@groupname or profile URL',
    facebookLabel: 'Facebook',
    facebookPlaceholder: 'Page name or Facebook URL',
    snapchatLabel: 'Snapchat',
    snapchatPlaceholder: 'Username or profile URL',
    tiktokLabel: 'TikTok',
    tiktokPlaceholder: '@groupname or profile URL',
  },
  detail: {
    title: 'Group Details',
    description: 'Description',
    members: 'Members',
    createdAt: 'Created',
    visibility: {
      public: 'Public',
      private: 'Private',
    },
  },
  memberships: {
    title: 'Members',
    noMembers: 'No members found',
    searchPlaceholder: 'Search members by name, role, or status...',
    invite: 'Invite Member',
    inviteMembers: 'Invite Members',
    inviteMember: 'Invite Member',
    inviteGuest: 'Invite Guest',
    inviteGuests: 'Invite Guests',
    inviteMembersDescription:
      'Search and select users to invite, then choose which roles they should start with.',
    inviteRoleDescription:
      'Tick one or more roles for invited people. The default invite role is preselected.',
    inviteMembersEmptyRoles: 'Create a role first before inviting members.',
    inviteGuestsDescription:
      'Invite users as guests with guest roles. Guests get access rights but are not official members.',
    inviteGuestsRoleDescription: 'Guest invitations must always include at least one guest role.',
    inviteGuestsEmptyRoles: 'Create a guest role first before inviting guests.',
    inviteSuccess: '{{count}} users invited successfully.',
    inviteSuccess_one: '{{count}} user invited successfully.',
    inviteSuccess_other: '{{count}} users invited successfully.',
    inviteGuestsSuccess: '{{count}} guests invited successfully.',
    inviteGuestsSuccess_one: '{{count}} guest invited successfully.',
    inviteGuestsSuccess_other: '{{count}} guests invited successfully.',
    selectedUserTitle: '{{count}} user',
    selectedUsersTitle: '{{count}} users',
    selectedUsersTitle_one: '{{count}} user',
    selectedUsersTitle_other: '{{count}} users',
    guestOnlyInviteDescription:
      'This sibling group only allows guest access invitations. Official member roles are not available here.',
    guestOnlyRoleDescription: 'Only guest roles can be used as invite defaults for this group.',
    guestOnlyEmptyRoles: 'Create a guest role first before inviting people to this group.',
    manage: 'Manage Memberships',
    tabs: {
      membershipsByUser: 'Memberships by user',
      membershipsByRole: 'Memberships by role',
      composition: 'Composition',
      rightsAlignment: 'Rights alignment',
      openAssignments: 'Open Assignments',
      guests: 'Guests',
      roles: 'Roles',
    },
    offlineRoster: {
      title: 'All users (incl. non signed-up offline users)',
      description:
        'Some real group members may never sign up on the platform. Use this roster to include those offline users, map them to active platform users when needed, and keep counts and delegate calculations grounded in the full real-world membership.',
      manageDialogTitle: 'Manage non signed up users',
      manageDialogDescription:
        'Add single offline users or import them from CSV so the full group roster remains complete.',
    },
    openAssignments: {
      title: 'Open Assignments',
      titleWithCount: 'Open Assignments ({{count}})',
      titleWithCount_one: 'Open Assignment ({{count}})',
      titleWithCount_other: 'Open Assignments ({{count}})',
      description:
        'Election, role, and amendment assignments for this group in the same table format as the other membership sections.',
      loadingDescription: "Loading this group's assignments.",
      emptyDescription: 'There are currently no open election, role, or amendment assignments.',
      columns: {
        assignment: 'Assignment',
        type: 'Type',
        status: 'Status',
        amendment: 'Amendment',
        events: 'Events',
        action: 'Action',
      },
      status: {
        completed: 'Completed',
        scheduled: 'Scheduled',
        open: 'Open',
      },
      type: {
        delegateElection: 'Delegate election',
        roleRenewal: 'Role renewal',
        implementationEvaluation: 'Implementation review',
        supportConfirmation: 'Support confirmation',
        processTask: 'Amendment process',
        assignment: 'Assignment',
      },
      seatCount: 'Seats',
      seatCount_one: 'Seat',
      seatCount_other: 'Seats',
      completedSeatCount: 'Elected',
      scheduledSeatCount: 'Scheduled',
      openSeatCount: 'Open',
      dueAt: 'Due',
      noAmendment: 'No amendment',
      targetEventLabel: 'Target',
      linkedEventLabel: 'Linked',
      noEventLinked: 'No event linked yet',
      completedBanner: 'Fully scheduled or completed',
      toScheduledElection: 'To scheduled election',
      showAgendaItems: 'Show new agenda items',
      thisGroup: 'this group',
      noEligibleEventsForGroup:
        'There is currently no upcoming or ongoing event for {{groupName}}.',
      createEvent: 'Create event',
      delegateElectionHelp:
        'Delegates must be elected at an upcoming or ongoing event for {{groupName}}.',
      delegateElectionHelpBeforeGroup:
        'Delegates must be elected at an upcoming or ongoing event for',
      delegateElectionHelpAfterGroup: '',
      delegateDescription: {
        sourceGroupFallback: 'This subgroup',
        targetGroupFallback: 'the target group',
        hasCurrently: 'currently has',
        seatSingular: 'delegate seat',
        seatPlural: 'delegate seats',
        seats: 'delegate seats',
        seats_one: 'delegate seat',
        seats_other: 'delegate seats',
        for: 'for',
      },
      searchDelegateElectionEvent: 'Find event for delegate election',
      searchRoleRenewalEvent: 'Find event for role election',
      roleRenewalHelp:
        'Choose an upcoming or ongoing event where this role election should be created.',
      selectEventLabel: 'Which event should this assignment be attached to?',
      selectEventPlaceholder: 'Select event',
      eventFallback: 'Event',
      scheduleImplementationReview: 'Schedule review',
      scheduleConfirmation: 'Schedule confirmation',
      attachToEvent: 'Attach to event',
      filters: {
        status: 'Status',
        assignmentKind: 'Type',
        all: 'All',
        allStatuses: 'All statuses',
        allKinds: 'All types',
        votes: 'Votes',
        elections: 'Elections',
        emptyTitle: 'No matching assignments',
        emptyDescription: 'Adjust the filters to show more assignments.',
      },
      generated: {
        amendmentFallback: 'Amendment',
        implementationEvaluationTitle: 'Review implementation: {{amendmentTitle}}',
        implementationEvaluationDescription:
          'Plan the implementation review for {{amendmentTitle}} in {{groupName}}.',
        supportConfirmationTitle: 'Confirm support: {{amendmentTitle}}',
        supportConfirmationDescription:
          'This group needs to confirm its support for {{amendmentTitle}} again.',
        scheduleAmendmentVoteTitle: 'Schedule amendment vote for {{groupName}}',
        scheduleAmendmentVoteDescription: 'No eligible event is selected yet for {{groupName}}.',
      },
      delegateDialog: {
        title: 'Find election event for delegate assignment',
        description:
          'Choose an upcoming or ongoing event for {{groupName}} where the delegate election should be created.',
        remainingSeats: '{{count}} delegates to elect',
        remainingSeats_one: '{{count}} delegate to elect',
        remainingSeats_other: '{{count}} delegates to elect',
        targetEvent: 'Target event for the delegate election:',
        searchLabel: 'Search upcoming or ongoing events',
        searchPlaceholder: 'Search by title',
        emptySearch: 'No upcoming or ongoing events were found for the current search.',
        cancel: 'Cancel',
        create: 'Create election',
      },
      roleRenewalDialog: {
        title: 'Find election event for role assignment',
        description:
          'Choose an upcoming or ongoing event where this role election should be created.',
        searchLabel: 'Search upcoming or ongoing events',
        searchPlaceholder: 'Search by title',
        emptySearch: 'No upcoming or ongoing events were found for the current search.',
        cancel: 'Cancel',
        create: 'Create election',
      },
    },
    composition: {
      title: 'Composition',
      description: 'Breakdown by subgroups. Leadership counts non-member role assignments.',
      modePercent: '%',
      modeAbsolute: 'Absolute',
      membersTitle: 'Members',
      membersDescription: 'Share of subgroups across all active memberships.',
      membersEmpty: 'No active members found.',
      leadershipTitle: 'Leadership',
      leadershipDescription: 'Share of subgroups across all non-member role assignments.',
      leadershipEmpty: 'No non-member role assignments found.',
      leadershipFootnote: 'Each non-member role is counted separately.',
      loading: 'Loading composition...',
      total: 'Total: {{count}}',
      directWithoutPath: 'Direct / no path',
    },
    rightsAlignment: {
      title: 'Rights alignment',
      description: 'Expected rights from connected group rights compared with actual role rights.',
      tableTitle: 'Member alignment',
      tableDescription: '{{count}} visible members',
      tableDescription_one: '{{count}} visible member',
      tableDescription_other: '{{count}} visible members',
      loading: 'Loading rights alignment...',
      empty: 'No memberships match this filter.',
      none: 'None',
      noOrigin: 'No origin',
      noConnectedRights: 'No connected rights',
      partGroup: 'Part',
      baseGroup: 'Base',
      filters: {
        all: 'All',
        missing: 'Missing',
        extra: 'Extra',
        mixed: 'Mixed',
        aligned: 'Aligned',
      },
      status: {
        aligned: 'Aligned',
        missing: 'Missing rights',
        extra: 'Extra rights',
        mixed: 'Missing and extra',
      },
      columns: {
        member: 'Member',
        origin: 'Origin',
        connectedRights: 'Connected rights',
        missing: 'Missing',
        extra: 'Extra',
        roles: 'Roles',
        actions: 'Actions',
      },
      actions: {
        rights: 'Rights',
        manageRoles: 'Manage roles',
      },
    },
    roles: {
      admin: 'Admin',
      member: 'Member',
      moderator: 'Moderator',
    },
    status: {
      pending: 'Pending',
      approved: 'Approved',
      rejected: 'Rejected',
    },
    actions: {
      approve: 'Approve',
      reject: 'Reject',
      remove: 'Remove',
      promote: 'Promote',
      demote: 'Demote',
    },
  },
  conflicts: {
    panel: {
      affectedGroupsTitle: 'Which groups are affected?',
      affectedUsersTitle: 'Which users are affected?',
      collidingSourceGroupsTitle: 'Which feeding groups are colliding?',
      collidingPathsTitle: 'Which paths are colliding?',
      baseGroupFallback: 'Base group',
      targetGroupFallback: 'Target group',
      resolutionTitle: 'How can this be resolved?',
      selfService: 'You can resolve this conflict yourself.',
      notSelfService: 'You cannot resolve this conflict yourself.',
      requiredRole: 'Responsible: {{role}}.',
    },
    dialog: {
      triggerLabel: 'Details',
      title: 'Conflict details',
      description:
        'Here you can see why the action is blocked and how the conflict can be resolved.',
      blockedSummaryFallback: 'Linking is blocked in this configuration.',
      blockedDescription: 'Resolve the conflict before saving the link.',
      blockedReasonTitle: 'Why is this link blocked?',
    },
    kinds: {
      hierarchyMemberOverlap: {
        summary: 'Linking would merge members from multiple subgroups of the same hierarchy.',
        explanation:
          'At least one person would then be active in multiple feeding subgroups of the same target hierarchy at the same time.',
        resolutions: {
          alignMemberships: {
            label: 'Align memberships',
            description:
              'Remove or deactivate overlapping memberships in one of the competing subgroups.',
          },
          contactOtherGroup: {
            label: 'Contact the other group',
            description:
              'If you cannot manage the competing subgroup yourself, the responsible admin side is required.',
          },
        },
      },
      hierarchyDuplicatePath: {
        summary: 'Linking would connect the same subtree twice.',
        explanation:
          'The same leaf base group would reach the target hierarchy via two active paths after activation.',
        resolutions: {
          removePath: {
            label: 'Remove or deactivate one path',
            description:
              'Linking is only possible when exactly one active path remains between the base group and the target hierarchy.',
          },
          contactResponsibleGroup: {
            label: 'Contact the responsible group',
            description:
              'If you cannot manage the competing path yourself, the affected hierarchy admin side is required.',
          },
        },
      },
    },
  },
  wiki: {
    membersTitle: 'Members',
    membersDescription: 'Active members visible in this group.',
    membersSearch: 'Search members',
    noMembers: 'No active members yet.',
    noMembersMatch: 'No members match your filters.',
  },
  roleDetails: {
    editRoleTitle: 'Edit Role',
    editRoleWithName: 'Edit {{roleName}}',
    derivedIncumbentTooltip: 'This incumbent is currently derived from membership roles.',
  },
  events: {
    title: 'Events',
    noEvents: 'No events found',
    searchPlaceholder: 'Search events...',
    hashtagPlaceholder: 'Enter hashtag to filter...',
    createEvent: 'Create Event',
  },
  amendments: {
    title: 'Amendments',
    noAmendments: 'No amendments found',
    searchPlaceholder: 'Search amendments...',
    createAmendment: 'Create Amendment',
  },
  common: {
    filters: {
      title: 'Filters',
      refine: 'Refine your search results',
      status: 'Filter by Status',
      allStatuses: 'All Statuses',
      hashtag: 'Filter by Hashtag',
      filteringBy: 'Filtering by:',
      active: 'Active filters:',
    },
    status: {
      acceptedApproved: 'Accepted / Approved',
      pending: 'Pending',
      rejected: 'Rejected',
      withdrawn: 'Withdrawn',
    },
  },
  network: {
    title: 'Network',
    searchPlaceholder: 'Search relationships by group name...',
    direction: 'Direction',
    filterAll: 'All',
    filterParent: 'Parent',
    filterChild: 'Child',
    noRelationships: 'No relationships found',
  },
  editor: {
    title: 'Documents',
    noDocuments: 'No documents found',
    searchPlaceholder: 'Search documents...',
    createDocument: 'Create Document',
  },
  operation: {
    title: 'Operation',
    description: 'Group operation settings',
  },
  notifications: {
    title: 'Notifications',
    description: 'Manage group notifications',
  },
  relationships: {
    title: 'Relationships',
    manage: 'Manage',
    remove: 'Remove',
    addRelationship: 'Add Relationship',
    parent: 'Parent Group',
    child: 'Child Group',
  },
  childGroups: {
    title: 'Child Groups',
    description: 'Groups that are subordinate to this group',
  },
  // Toast messages for action hooks
  toasts: {
    created: 'Group created',
    updated: 'Group updated',
    deleted: 'Group deleted',
    createFailed: 'Failed to create group',
    updateFailed: 'Failed to update group',
    deleteFailed: 'Failed to delete group',
    joined: 'Joined group',
    joinFailed: 'Failed to join group',
    left: 'Left group',
    leaveFailed: 'Failed to leave group',
    invitationSent: 'Invitation sent',
    inviteFailed: 'Failed to invite member',
    invitationAccepted: 'Invitation accepted',
    acceptInvitationFailed: 'Failed to accept invitation',
    memberRoleUpdated: 'Member role updated',
    memberRoleUpdateFailed: 'Failed to update member role',
    roleCreated: 'Role created',
    roleCreateFailed: 'Failed to create role',
    roleDeleted: 'Role deleted',
    roleDeleteFailed: 'Failed to delete role',
    roleHolderHistoryCreated: 'Incumbent history created',
    roleHolderHistoryCreateFailed: 'Failed to create incumbent history',
    roleHolderHistoryUpdated: 'Incumbent history updated',
    roleHolderHistoryUpdateFailed: 'Failed to update incumbent history',
    actionRightAssigned: 'Action right assigned',
    actionRightAssignFailed: 'Failed to assign action right',
    actionRightRemoved: 'Action right removed',
    actionRightRemoveFailed: 'Failed to remove action right',
    relationshipCreated: 'Relationship created',
    relationshipCreateFailed: 'Failed to create relationship',
    relationshipUpdated: 'Relationship updated',
    relationshipUpdateFailed: 'Failed to update relationship',
    relationshipDeleted: 'Relationship deleted',
    relationshipDeleteFailed: 'Failed to delete relationship',
  },
  list: {
    searchPlaceholder: 'Search groups by name, description, or tags...',
    noGroups: 'No groups found',
    noGroupsDescription: 'Try adjusting your search terms or clearing the filters.',
    loading: 'Loading groups...',
    groupsFound: '{{count}} groups found',
    groupsFound_one: '{{count}} group found',
    groupsFound_other: '{{count}} groups found',
    filters: {
      all: 'All',
      myGroups: 'My Groups',
      public: 'Public',
      filterByTags: 'Filter by tags:',
      clearFilters: 'Clear filters',
      activeFilters: 'Active filters:',
      searchLabel: 'Search: "{{query}}"',
    },
  },
} as const;
