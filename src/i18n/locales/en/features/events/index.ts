export const eventsTranslations = {
  title: 'Events',
  description: 'Manage and participate in events',
  create: 'Create Event',
  edit: 'Edit Event',
  delete: 'Delete Event',
  join: 'Join Event',
  leave: 'Leave Event',
  backToCalendar: 'Back to Calendar',
  cancelLabel: 'Cancel',
  add: 'Add',
  editPage: {
    title: 'Edit Event',
    untitled: 'Untitled Event',
    subtitle: 'Update event information',
    loading: 'Loading event data...',
    notFound: 'Event not found',
    notFoundDescription: 'No event data exists for this ID',
    saveChanges: 'Save Changes',
    saving: 'Saving...',
    finalVoteDurationLabel: 'Default final vote duration',
    finalVoteDurationNoAutoClose: 'No automatic close',
    finalVoteDurationHint:
      'Used as the default close timer for final votes created from this event agenda.',
    finalVoteDurationInvalid: 'Enter a valid final vote duration.',
    basicInfo: {
      title: 'Basic Information',
      description: 'Public event information',
    },
    eventTitle: 'Event Title *',
    eventTitlePlaceholder: 'Event title',
    eventDescription: 'Description',
    eventDescriptionPlaceholder: 'Describe the event and its purpose...',
    publicEvent: 'Public Event',
    dateTime: {
      title: 'Date & Time',
      description: 'When is this event happening?',
      startDate: 'Start Date *',
      startTime: 'Start Time',
      endDate: 'End Date',
      endTime: 'End Time',
    },
    timeSeries: {
      title: 'Time & Series',
      description: 'Plan the schedule, recurrence, and time-based deadlines in one place.',
      validation: {
        startDateRequired: 'Recurring events need a start date.',
        weekdaysRequired: 'Pick at least one weekday for a weekly series.',
      },
    },
    locationCapacity: {
      title: 'Location & Capacity',
      description: 'Where is this event and how many can attend?',
      attendanceModeLockedLabel: 'Why is the attendance mode locked?',
      attendanceModeLockedDescription:
        'The attendance mode cannot be changed because a final vote or election has already started. Close it first.',
      location: 'Location',
      locationPlaceholder: 'Event location or venue',
      capacity: 'Capacity (optional)',
      capacityPlaceholder: 'Maximum number of participants',
    },
    tags: {
      title: 'Tags',
      description: 'Add tags to categorize this event',
      placeholder: 'Add a tag',
    },
    image: {
      label: 'Event Image',
      description: 'Upload an event image or provide a URL',
    },
  },
  detail: {
    title: 'Event Details',
    date: 'Date',
    time: 'Time',
    location: 'Location',
    organizer: 'Organizer',
    participants: 'Participants',
    maxParticipants: 'Max Participants',
    description: 'Description',
    status: {
      upcoming: 'Upcoming',
      ongoing: 'Ongoing',
      past: 'Past',
      cancelled: 'Cancelled',
    },
  },
  backToEvent: 'Back to Event',
  timeline: {
    ariaLabel: 'Event timeline',
    title: 'Schedule',
    eventStart: 'Event start',
    eventEnd: 'Event end',
  },
  agenda: {
    title: 'Agenda',
    backToAgenda: 'Back to agenda',
    speakerList: 'Speaker list',
    amendmentDetails: 'Amendment context',
    viewAmendment: 'View amendment',
    speakerListDescription: 'Select a speaker to view their profile.',
    speakerListEmpty: 'No speakers yet',
    speakerCount: '{{count}} speakers',
    speakerCount_one: '{{count}} speaker',
    speakerCount_other: '{{count}} speakers',
    unspecifiedSpeaker: 'not specified',
    noItems: 'No agenda items',
    noItemsDescription: 'No agenda items have been created for this event yet.',
    createFirstItem: 'Create first agenda item',
    addItem: 'Add Agenda Item',
    statistics: 'Agenda Statistics',
    elections: 'Elections',
    election: 'Election',
    amendments: 'Amendments',
    amendment: 'Amendment',
    votes: 'Votes',
    vote: 'Vote',
    openChangeRequests: 'Open Change Requests',
    itemsCount: 'Agenda Items ({{count}})',
    itemsCount_one: 'Agenda Item ({{count}})',
    itemsCount_other: 'Agenda Items ({{count}})',
    searchPlaceholder: 'Search agenda items (e.g. TOP-1 or title)...',
    filters: 'Filters',
    filtersDescription: 'Refine the agenda items',
    type: 'Type',
    allTypes: 'All Types',
    typeElection: 'Election',
    typeVote: 'Vote',
    typeSpeech: 'Speech',
    typeDiscussion: 'Discussion',
    typeAccreditation: 'Accreditation',
    statusLabel: 'Status',
    allStatus: 'All Status',
    statusPending: 'Pending',
    statusInProgress: 'In Progress',
    statusCompleted: 'Completed',
    statusPlanned: 'Planned',
    active: 'Active',
    details: 'Details',
    by: 'By {{name}}',
    unspecified: 'not specified',
    itemActivated: 'Agenda Item Activated',
    startReminderTitle: 'Start overdue agenda item',
    startReminderDescription:
      'The next agenda item is overdue. Start it now to keep the live stream and badges in sync.',
    // Context card
    scheduledFor: 'Scheduled for',
    startedAt: 'Started at',
    completedAt: 'Completed at',
    estimatedStartAt: 'Estimated to start at',
    estimatedCompleteAt: 'Estimated to complete at',
    endsIn: 'Ends in',
    duration: 'Duration',
    minutes: 'minutes',
    electionFor: 'Election for',
    voteOn: 'Vote on',
    notScheduled: 'Not scheduled',
    notStarted: 'Not started',
    notCompleted: 'Not completed',
    timeline: 'Agenda timeline',
    editItemDescriptionPlaceholder: 'Add context for this agenda item...',
    // Speaker section
    currentSpeaker: 'Current Speaker',
    waitingQueue: 'Waiting Queue',
    joinSpeakerList: 'Join Speaker List',
    joiningSpeakerList: 'Joining speaker list',
    leaveSpeakerList: 'Leave Speaker List',
    leavingSpeakerList: 'Leaving speaker list',
    alreadyOnList: 'Already on list',
    markCompleted: 'Mark Completed',
    noSpeakersYet: 'No speakers yet',
    speakerPosition: 'Position {{count}}',
    turnStartsIn: 'Turn starts in',
    timeRemainingLabel: 'Time remaining',
    turnCompletedLabel: 'Turn completed',
    completedSpeaker: 'Completed',
    estimatedStartLabel: 'Estimated at',
    upNext: 'Up next',
    hideSpeakerList: 'Hide speaker list',
    showSpeakerList: 'Show speaker list',
    userSpeakerCurrentSummary: 'You are speaking now. {{time}} remaining.',
    userSpeakerQueueSummary: 'Position {{position}}. {{time}} until your speech starts.',
    userSpeakerPendingSummary: 'Your speaker entry is active.',
    youSpeakerPosition: 'You: #{{position}}',
    genderQuota: {
      settingsLabel: 'Gender-quoted speaker list',
      settingsDescription: 'When enabled, male and female speakers must alternate.',
      missingGender:
        'Please choose a gender in your profile settings before joining this speaker list.',
      unsupportedGender: 'This gender-quoted speaker list currently only accepts male or female.',
      expectedMale: 'The next speaker must be male.',
      expectedFemale: 'The next speaker must be female.',
      blocked: 'You cannot join this gender-quoted speaker list right now.',
      genderLabels: {
        male: 'male',
        female: 'female',
        diverse: 'diverse',
        unspecified: 'not specified',
      },
    },
    changeRequestVoteOrder: {
      settingsLabel: 'Change request voting order',
      textPosition: 'Text position',
      changedCharacters: 'Changed characters',
      crNumber: 'CR number',
      text_position: 'Text position',
      changed_character_count: 'Changed characters',
      cr_number: 'CR number',
    },
    // Vote section
    voteAndResults: 'Vote & Results',
    changeRequests: 'Change Requests',
    voteChangeRequestsFirst: 'Vote on all change requests before the final vote',
    finalVote: 'Final Vote',
    voteYes: 'Yes',
    voteNo: 'No',
    voteAbstain: 'Abstain',
    voteYesLabel: 'Yes',
    voteNoLabel: 'No',
    voteAbstainLabel: 'Abstain',
    castVote: 'Cast Vote',
    changeVote: 'Change Vote',
    indicateVote: 'Indicate Vote',
    confirmVote: 'Confirm Your Vote',
    confirmIndication: 'Confirm Your Indication',
    yourVote: 'Your vote',
    yourIndication: 'Your indication',
    voteResults: 'Vote Results',
    votingWorkspaceDescription:
      'Voting progress, decisions, and results for the active agenda item.',
    noChoices: 'No choices defined',
    selected: 'Selected',
    winner: 'Winner',
    openNamedResults: 'Open named results',
    noVotesYet: 'No votes yet',
    activate: 'Activate',
    defaultChoiceLabels: {
      yes: 'Yes',
      no: 'No',
      abstain: 'Abstain',
      choice: 'Choice',
      choiceWithNumber: 'Choice {{count}}',
    },
    namedResults: {
      label: 'Named',
      title: 'Named results',
      totalResult: 'Total result',
      offlineShare: '{{count}} offline',
      notParticipated: 'not participated',
      pending: 'pending',
      recorded: 'recorded',
      offlineAggregated: 'offline aggregated',
      unavailable: 'No named results available',
      unnamedSourceGroup: 'Unnamed source group',
      electionDescription: 'Named results for the current election.',
      voteDescription: 'Named results for the current vote.',
      electionFallbackTitle: 'Named election',
      voteFallbackTitle: 'Named vote',
    },
    forwarding: {
      pendingPrefix: 'The amendment will be forwarded to',
      pendingSuffix: ' after the vote.',
      completedPrefix: 'The amendment was successfully forwarded to',
      completedSuffix: '.',
      rejectedPrefix: 'The amendment was not forwarded to',
      rejectedSuffix: ' because the vote was rejected.',
      tiePrefix: 'The amendment was not forwarded to',
      tieSuffix: ' because the vote ended in a tie.',
    },
    // Indication system
    indication: 'Indication',
    indicationShort: 'Ind',
    actual: 'Actual',
    actualShort: 'Act',
    indicationOnly: 'Indication only',
    indicationVotes: 'indication votes',
    showIndicationResults: 'Show indication results',
    hideIndicationResults: 'Hide indication results',
    // Election section
    electionResults: 'Election Results',
    noCandidates: 'No candidates yet',
    delegateParticipantsAdded:
      'Elected delegates were automatically added to the event as participants.',
    roleWinnersAssigned: 'The roles were automatically assigned to the winners.',
    eventRoleWinnersAssigned: 'The roles were automatically assigned to the winners.',
    candidateNominated: 'Nominated',
    candidateAccepted: 'Accepted',
    candidateWithdrawn: 'Withdrawn',
    castIndication: 'Cast Indication',
    selectCandidate: 'Select a Candidate',
    selectCandidateIndication: 'Select a Candidate (Indication)',
    becomeCandidate: 'Become Candidate',
    becomeCandidateTitle: 'Become a Candidate',
    becomeCandidateDescription: 'You are about to become a candidate for the role: {{role}}',
    withdrawCandidacy: 'Withdraw Candidacy',
    quickActions: {
      title: 'Quick Actions',
      description: 'Manage the agenda for this event',
      addItem: 'Add Agenda Item',
      createElection: 'Create Election',
      createVote: 'Create Vote',
      backToEvent: 'Back to Event',
    },
    item: {
      title: 'Title',
      description: 'Description',
      duration: 'Duration',
      presenter: 'Presenter',
    },
    // Transfer/Move agenda items
    transferItem: 'Transfer Agenda Item',
    moveToEvent: 'Move to Another Event',
    selectDestinationEvent: 'Select the event you want to move this agenda item to.',
    searchEvents: 'Search for an event...',
    transferConfirm: 'Transfer',
    transferring: 'Transferring...',
    transferSuccess: 'Agenda item transferred successfully',
    transferError: 'Failed to transfer agenda item',
    currentEvent: 'Current Event',
    destinationEvent: 'Destination Event',
    transferWarning: 'All participants of both events will be notified about this transfer.',
    noEventsWithPermission: 'No events found where you have manage agenda rights.',
    // Delete agenda item
    deleteTitle: 'Delete Agenda Item?',
    deleteDescription:
      'This action cannot be undone. The agenda item and all associated data will be permanently deleted.',
    candidate: 'Candidate',
    completeCurrentItem: 'Complete current item',
    defaultCandidateWithNumber: 'Candidate {{count}}',
    manageOfflineTally: 'Manage offline tally',
    nextAgendaItem: 'Next agenda item',
    offlineTallyDescription: 'Manage offline ballots for {{item}}.',
    roleLowercase: 'role',
    role: 'Role',
    roleDetails: 'Role Details',
    thisElection: 'this election',
    thisItem: 'this item',
    thisVote: 'this vote',
    voteSubmitted: 'Vote submitted',
    votingEnd: 'Voting end',
    votingStart: 'Voting start',
    viewGroup: 'View group',
    term: 'Term',
    actions: {
      toolbar: 'Agenda controls',
      becomeCandidate: 'Become candidate',
      candidateRequiresPassiveVotingRight: 'Passive voting rights are required to stand.',
      castFinalVote: 'Cast final vote',
      castIndicativeVote: 'Cast indication',
      closeFinalVote: 'Close final vote',
      joinSpeakerList: 'Join speaker list',
      leaveSpeakerList: 'Leave speaker list',
      offlineCRVoteUnavailable: 'Offline voting is unavailable for this change request.',
      secretIndicativeVoteAlreadyCast: 'You have already cast a secret indication.',
      startFinalVote: 'Start final vote',
      startVote: 'Start vote',
      voteRequiresActiveVotingRight: 'Active voting rights are required to vote.',
      voteUnavailable: 'Voting is unavailable.',
      withdrawCandidacy: 'Withdraw candidacy',
    },
  },
  participants: {
    title: 'Participants',
    manageParticipants: 'Manage Participants',
    event: 'Event',
    noParticipants: 'No participants yet',
    searchPlaceholder: 'Search participants by name, email, or status...',
    back: 'Back',
    invite: 'Invite Participant',
    inviteTitle: 'Invite Participants',
    inviteDescription:
      'Search and select users to invite to this event. They will receive an invitation to participate.',
    searchUsersPlaceholder: 'Search by name, handle, or email...',
    noUsersFound: 'No users found.',
    selected: 'Selected ({{count}})',
    inviting: 'Inviting...',
    inviteCount: 'Invite ({{count}})',
    loading: 'Loading...',
    notFound: 'Event not found',
    tabs: {
      participants: 'Participants',
      composition: 'Composition',
      roles: 'Roles',
    },
    composition: {
      title: 'Delegate Composition',
      description: 'Seat distribution by subgroup for this delegate assembly.',
      loading: 'Loading composition...',
      empty: 'No delegate seats have been allocated to subgroups yet.',
      total: '{{count}} total seats',
      total_one: '{{count}} total seat',
      total_other: '{{count}} total seats',
      metrics: {
        planned: 'Planned',
        scheduled: 'Votes scheduled',
        elected: 'Elected',
      },
      metricTitles: {
        planned: 'Planned Delegate Seats',
        scheduled: 'Scheduled Elections',
        elected: 'Already Elected Delegates',
      },
      metricDescriptions: {
        planned: 'Seats from the current subgroup delegate allocation.',
        scheduled: 'Seats that already have scheduled subgroup delegate elections.',
        elected: 'Confirmed delegates on this delegate assembly.',
      },
      sectionTitles: {
        planned: 'Planned delegates',
        scheduled: 'Scheduled delegates',
        elected: 'Elected delegates',
      },
      sectionDescriptions: {
        planned: 'Seats from the current subgroup delegate allocation.',
        scheduled: 'Seats with scheduled subgroup elections plus seats not yet scheduled.',
        elected: 'Confirmed delegates plus seats not yet elected.',
      },
      remainder: {
        unscheduled: 'Unscheduled',
        notYetElected: 'Not yet elected',
      },
      columns: {
        category: 'Category',
        group: 'Subgroup',
        absolute: 'Absolute',
        planned: 'Planned',
        scheduled: 'Scheduled',
        elected: 'Elected',
        share: 'Share',
      },
    },
    participantComposition: {
      participantsTitle: 'Participants',
      participantsDescription: 'Share of part groups across all active/offline event participants.',
      participantsEmpty: 'No participants found.',
      noBaseGroup: 'No base group',
    },
    pendingRequests: 'Pending Requests ({{count}})',
    pendingRequests_one: 'Pending Request ({{count}})',
    pendingRequests_other: 'Pending Requests ({{count}})',
    activeParticipants: 'Active Participants ({{count}})',
    activeParticipants_one: 'Active Participant ({{count}})',
    activeParticipants_other: 'Active Participants ({{count}})',
    pendingInvitations: 'Pending Invitations ({{count}})',
    pendingInvitations_one: 'Pending Invitation ({{count}})',
    pendingInvitations_other: 'Pending Invitations ({{count}})',
    inviteSuccess: 'Successfully invited {{count}} participants',
    inviteSuccess_one: 'Successfully invited {{count}} participant',
    inviteSuccess_other: 'Successfully invited {{count}} participants',
    table: {
      user: 'User',
      email: 'Email',
      role: 'Role',
      requested: 'Requested',
      joined: 'Joined',
      invited: 'Invited',
      actions: 'Actions',
    },
    actions: {
      accept: 'Accept',
      decline: 'Decline',
      makeOrganizer: 'Make Organizer',
      removeOrganizer: 'Remove Organizer',
      remove: 'Remove',
      cancelInvitation: 'Cancel Invitation',
    },
    roles: {
      title: 'Role Permissions',
      description: 'Manage roles and their action rights for this event',
      addRole: 'Add Role',
      addRoleTitle: 'Add New Role',
      addRoleDescription: 'Create a new role with custom permissions for this event.',
      roleName: 'Role Name',
      roleNamePlaceholder: 'e.g., Organizer, Speaker, Moderator',
      roleDescription: 'Description (Optional)',
      roleDescriptionPlaceholder: "Describe this role's purpose",
      createRole: 'Create Role',
      noRoles: "No roles created yet. Click 'Add Role' to create your first role.",
      actionRight: 'Action Right',
    },
    status: {
      registered: 'Registered',
      attended: 'Attended',
      cancelled: 'Cancelled',
    },
  },
  roles: {
    title: 'Roles',
    noRoles: 'No roles defined',
    addRole: 'Add Role',
    currentHolder: 'Current Holder',
    currentHolders: 'Current Holders',
    vacantRole: 'Vacant Role',
    election: 'Election',
    guestVotingRightsDisabled:
      'Guest voting rights are disabled for this event. Assign participants to a role instead.',
    scheduled: 'Scheduled',
  },
  network: {
    title: 'Network',
    description: 'View event connections',
  },
  stream: {
    title: 'Stream',
    startStream: 'Start Stream',
    endStream: 'End Stream',
    viewers: 'Viewers',
    live: 'LIVE',
    liveFocus: 'Live focus',
    liveFocusDescription: 'Focused view for the current agenda item, speakers, and vote.',
    liveStream: 'Event Live Stream',
    externalStreamDescription: 'This provider cannot be embedded here. Open the stream directly.',
    openStream: 'Open stream',
    noActiveItem: 'No active agenda item found',
    openLiveFocus: 'Open live focus',
    speakersList: 'Speaker list',
    startsIn: 'Starts in',
    viewDetails: 'View Details',
    votingEnd: 'Voting end',
    votingStart: 'Voting start',
  },
  notifications: {
    title: 'Notifications',
    description: 'Manage event notifications',
  },
  wiki: {
    loading: 'Loading event...',
    notFound: 'Event Not Found',
    notFoundDescription: "The event you're looking for doesn't exist or has been removed.",
    runForOffice: 'Run for Office',
    deadlines: 'Deadlines',
    delegateNomination: 'Delegate Nomination:',
    proposalSubmission: 'Proposal Submission:',
    amendmentDeadline: 'Amendment Deadline:',
    onlineMeeting: 'Online Meeting',
    eventLocation: 'Event Location',
    meetingLink: 'Meeting Link:',
    accessCode: 'Access Code:',
    clickToView: 'Click to view list',
    participantsCount: '{{count}} participants',
    participantsCount_one: '{{count}} participant',
    participantsCount_other: '{{count}} participants',
    eventParticipants: 'Event Participants',
    participantsRegistered: '{{count}} participants registered for this event',
    participantsRegistered_one: '{{count}} participant registered for this event',
    participantsRegistered_other: '{{count}} participants registered for this event',
    noParticipantsMatch: 'No participants match your search.',
    delegates: 'Delegates',
    delegatesBySubgroup: 'Delegates by Subgroup',
    selectElection: 'Select an Election',
    selectElectionDescription: 'Select an election you would like to run for.',
    alreadyCandidate: 'Already Candidate',
    candidates: 'Candidates:',
    candidatesCount: '{{count}} candidates',
    candidatesCount_one: '{{count}} candidate',
    candidatesCount_other: '{{count}} candidates',
    confirmCandidacy: 'Confirm Candidacy',
    confirmCandidacyQuestion: 'Do you really want to run for this election?',
    role: 'Role:',
    currentCandidates: 'Current Candidates:',
    votingMethod: 'Voting Method:',
    cancel: 'Cancel',
    confirm: 'Confirm Candidacy',
    submitting: 'Submitting...',
    candidacySuccess: 'You have been successfully added as a candidate!',
    candidacyError: 'Error adding candidate. Please try again.',
    alreadyCandidateError: 'You are already a candidate for this election',
    recurring: {
      daily: 'Daily',
      weekly: 'Weekly',
      monthly: 'Monthly',
      yearly: 'Yearly',
      fourYearly: 'Every 4 Years',
    },
  },
  candidacy: {
    becomeTitle: 'Confirm candidacy',
    becomeDescription: 'Enter your voting PIN to become a candidate for this election.',
    withdrawTitle: 'Withdraw candidacy',
    withdrawDescription: 'Enter your voting PIN to withdraw your candidacy from this election.',
    loadingDescription: 'Polity is confirming your PIN and updating the election.',
    role: 'Role',
    currentCandidates: 'Current candidates',
    currentCandidates_one: 'Current candidate',
    currentCandidates_other: 'Current candidates',
    votingMethod: 'Voting method',
    verifyingPin: 'Verifying PIN...',
    loadingStepsLabel: 'Candidacy confirmation progress',
    stepVerifyPin: 'Verify PIN',
    stepSubmitCandidacy: 'Submit candidacy',
    stepWithdrawCandidacy: 'Withdraw candidacy',
    stepSyncElection: 'Sync election',
    cancel: 'Cancel',
  },
  // Toast messages for action hooks
  toasts: {
    created: 'Event created',
    createFailed: 'Failed to create event',
    updateFailed: 'Failed to update event',
    cancelled: 'Event cancelled',
    cancelFailed: 'Failed to cancel event',
    joined: 'Joined event',
    joinFailed: 'Failed to join event',
    participantInvited: 'Participant invited',
    inviteFailed: 'Failed to invite participant',
    left: 'Left event',
    leaveFailed: 'Failed to leave event',
    updateParticipantFailed: 'Failed to update participant',
    agendaItemFinalized: 'Agenda item finalized',
    agendaItemFinalizeFailed: 'Failed to finalize agenda item',
    castVoteFailed: 'Failed to cast vote',
    votingSessionStarted: 'Voting session started',
    votingSessionStartFailed: 'Failed to start voting session',
    votingSessionEnded: 'Voting session ended',
    votingSessionEndFailed: 'Failed to end voting session',
    delegatesFinalized: 'Delegates finalized',
    delegatesFinalizeFailed: 'Failed to finalize delegates',
    roleCreated: 'Role created',
    roleCreateFailed: 'Failed to create role',
    roleUpdateFailed: 'Failed to update role',
    roleDeleted: 'Role deleted',
    roleDeleteFailed: 'Failed to delete role',
    meetingSlotCreated: 'Meeting slot created',
    meetingSlotCreateFailed: 'Failed to create meeting slot',
    meetingSlotUpdateFailed: 'Failed to update meeting slot',
    meetingSlotDeleted: 'Meeting slot deleted',
    meetingSlotDeleteFailed: 'Failed to delete meeting slot',
    meetingBooked: 'Meeting booked',
    meetingBookFailed: 'Failed to book meeting',
    bookingCancelled: 'Booking cancelled',
    bookingCancelFailed: 'Failed to cancel booking',
  },
  list: {
    searchPlaceholder: 'Search events...',
    noEvents: 'No events found',
    filters: {
      all: 'All',
      upcoming: 'Upcoming',
      past: 'Past',
      myEvents: 'My Events',
    },
  },
  // Agenda Navigation
  navigation: {
    title: 'Navigation',
    progress: 'Progress',
    of: 'of',
    currentItem: 'Current Item',
    noItems: 'No agenda items',
    start: 'Start',
    previous: 'Previous',
    next: 'Next',
    complete: 'Complete',
    completed: 'Completed',
    notActivated: 'Not activated',
    activating: 'Activating...',
    completing: 'Completing...',
    navigating: 'Navigating...',
  },
  // Voting System
  voting: {
    title: 'Voting',
    session: 'Voting Session',
    startVoting: 'Start Voting',
    endVoting: 'End Voting',
    castVote: 'Cast Vote',
    yourVote: 'Your Vote',
    accept: 'Accept',
    reject: 'Reject',
    abstain: 'Abstain',
    accepted: 'Accepted',
    rejected: 'Rejected',
    abstained: 'Abstained',
    votes: 'Votes',
    eligible: 'Eligible',
    voted: 'Voted',
    share: 'Share',
    resultsLabel: 'Results',
    passed: 'Passed',
    failed: 'Failed',
    pending: 'Pending',
    inProgress: 'In Progress',
    closed: 'Closed',
    quorum: 'Quorum',
    quorumReached: 'Quorum reached',
    quorumNotReached: 'Quorum not reached',
    timeRemaining: 'Time remaining',
    seconds: 'seconds',
    minutes: 'minutes',
    votingClosed: 'Voting closed',
    voteRecorded: 'Vote recorded',
    alreadyVoted: 'Already voted',
    changeVote: 'Change vote',
    setup: {
      title: 'Setup Voting',
      duration: 'Duration (seconds)',
      quorumPercentage: 'Quorum percentage',
      passThreshold: 'Pass threshold (%)',
      startIntroduction: 'Start Introduction',
      startVoting: 'Start Voting',
    },
    phases: {
      setup: 'Setup',
      introduction: 'Introduction',
      internal: 'Internal vote',
      voting: 'Voting',
      indication: 'Indication',
      finalVote: 'Final Vote',
      closed: 'Closed',
    },
    results: {
      title: 'Voting Result',
      total: 'Total',
      forVotes: 'For',
      againstVotes: 'Against',
      abstentions: 'Abstentions',
      percentage: 'Percentage',
    },
  },
  // Event Cancellation
  cancel: {
    title: 'Cancel Event',
    description: 'Cancel this event and optionally reassign agenda items.',
    confirm: 'Cancel Event',
    warning: {
      title: 'Warning: This action cannot be undone',
      description: 'All participants will be notified and the event will be marked as cancelled.',
    },
    reason: {
      label: 'Cancellation Reason',
      placeholder: 'Provide a reason for cancelling this event...',
    },
    reassign: {
      label: 'Reassign Agenda Items',
      amendment: 'Amendment',
      election: 'Election',
      targetEvent: 'Target Event',
      selectEvent: 'Select event for reassignment',
      noEvents: 'No future events available',
      itemCount: 'items will be reassigned',
      itemCount_one: 'item will be reassigned',
      itemCount_other: 'items will be reassigned',
    },
  },
} as const;
