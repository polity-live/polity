export const ALPHA_WARNING_VERSION = '0.12.1';
export const ALPHA_WARNING_SESSION_KEY = `polity.alphaWarning.${ALPHA_WARNING_VERSION}.acknowledged`;

export const GITHUB_REPOSITORY_PATH = 'polity-live/polity';
export const GITHUB_REPOSITORY_URL = `https://github.com/${GITHUB_REPOSITORY_PATH}`;
export const GITHUB_ISSUES_URL = `${GITHUB_REPOSITORY_URL}/issues`;
export const SUPPORT_EMAIL = 'team@polity.live';
