...
import { Sheet, SheetContent } from '@/features/shared/ui/ui/sheet.tsx';
import { Skeleton } from '@/features/shared/ui/ui/skeleton.tsx';
import {
  Tooltip,
  TooltipContent,
  TooltipHint,
...
>(
  (
    ...
    const toggleSidebar = React.useCallback(() => {
      return isMobileScreen ? setOpenMobile(open => !open) : setOpen(open => !open);
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    ...
    >
      <PanelLeft />
      <span className="sr-only">{t('plateJs.toolbar.toggleSidebar')}</span>
    </Button>
  );
});
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<'button'>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <TooltipHint content={t('plateJs.toolbar.toggleSidebar')}>
        <button
          ref={ref}
          data-sidebar="rail"
          aria-label={t('plateJs.toolbar.toggleSidebar')}
          tabIndex={-1}
