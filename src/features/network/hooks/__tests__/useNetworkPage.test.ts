...
describe('useNetworkPage request actions', () => {
  ...
  it('filters active and grouped request rows by direction, search text, and selected rights', () => {
    const currentGroup = { id: 'group-1', name: 'Current Group' };
    const partnerGroup = { id: 'group-2', name: 'Partner Group' };
    const structureRel = createRelationship({
      id: 'mixed:structure',
      connection_request_id: 'mixed',
      grant_id: null,
      membership_request_id: null,
      request_item_kind: 'structure',
    ...
    const duplicateStructureRel = createRelationship({
      ...structureRel,
      id: 'mixed:structure-duplicate',
    });
    const membershipRel = createRelationship({
      id: 'mixed:membership',
      connection_request_id: 'mixed',
      grant_id: null,
      membership_request_id: 'membership-1',
      request_item_kind: 'membership',
      membership_mode: 'all_members',
      group_id: 'group-1',
      related_group_id: 'group-2',
    ...
    const implicitRightRel = createRelationship({
      id: 'mixed:right',
      connection_request_id: 'mixed',
      request_item_kind: null,
      group_id: 'group-1',
      related_group_id: 'group-2',
      parent_group_id: 'group-1',
      child_group_id: 'group-2',
    ...
    const rightWithoutTypeRel = createRelationship({
      id: 'mixed:right-without-type',
      connection_request_id: 'mixed',
      grant_id: 'right-without-type',
      request_item_kind: 'right',
      with_right: null,
      group_id: 'group-1',
      related_group_id: 'group-2',
    ...
    const anonymousStructureRel = createRelationship({
      ...
      request_item_kind: 'structure',
      group_id: 'group-1',
      related_group_id: 'group-3',
      parent_group_id: 'group-1',
      child_group_id: 'group-3',
      group: currentGroup,
      related_group: { id: 'group-3', name: 'Anonymous Group' },
    });
    const rightOnlyRel = createRelationship({
      id: 'right-only',
      connection_request_id: 'right-only',
      group_id: 'group-1',
      related_group_id: 'group-4',
      parent_group_id: 'group-1',
      child_group_id: 'group-4',
      group: currentGroup,
      related_group: { id: 'group-4', name: 'Right Only Group' },
    });
    ...
    const requests = [
      structureRel,
      duplicateStructureRel,
      membershipRel,
      implicitRightRel,
      rightWithoutTypeRel,
      anonymousStructureRel,
      rightOnlyRel,
      invalidRel,
    ];
    ...
    useGroupNetworkMock.mockReturnValue({
      networkData: {
        parents: [
          {
            group: { id: 'parent', name: 'Parent Match', description: 7 },
        ...
        children: [
          {
            group: { id: 'child-info', name: 'Info Child', description: 'Description match' },
            rights: ['informationRight'],
            type: 'child',
          },
          {
            group: { id: 'child-amendment', name: undefined, description: 'Description match' },
      ...
      allRelationships: requests,
      groupConnections: [],
      isLoading: false,
    });

    const { result } = renderHook(() => useNetworkPage('group-1'));
    expect(result.current.filteredIncoming).toHaveLength(3);
    expect(result.current.incomingRequestCount).toBe(3);
    expect(result.current.filteredIncoming[0]).toMatchObject({
      membershipMode: 'all_members',
      structureRel,
    });

    act(() => result.current.setSearchQuery('description match'));
    expect(result.current.filteredRelationships).toHaveLength(2);
    expect(result.current.filteredIncoming).toEqual([]);
    expect(result.current.filteredOutgoing).toEqual([]);

    act(() => {
      result.current.setSearchQuery('');
      result.current.setDirectionFilter('parent');
      result.current.toggleManageRightFilter('informationRight');
    });
    expect(result.current.filteredRelationships.map(item => item.group.id)).toEqual([
      'child-amendment',
    ]);
    expect(result.current.filteredIncoming).toHaveLength(2);
    expect(result.current.filteredIncoming[0].rightRels).toEqual([]);

    act(() => result.current.toggleManageRightFilter('informationRight'));
    expect(result.current.filteredRelationships).toHaveLength(2);
  });

  it('guards blocked and incomplete requests and reports asynchronous approval failures', async () => {
    const approveGroupConnectionRequest = vi
      .fn()
      .mockReturnValueOnce({ server: Promise.resolve({ type: 'error' }) })
      .mockReturnValueOnce({
        server: Promise.resolve({ type: 'error', error: { message: 'server failure' } }),
      });
    const rejectGroupConnectionRequest = vi.fn();
    useGroupConnectionActionsMock.mockReturnValue({
      approveGroupConnectionRequest,
      rejectGroupConnectionRequest,
      deleteGroupConnection: vi.fn(),
    });
    const canActivateLink = vi.fn(() => false);
    useHierarchyLinkConflictsMock.mockReturnValue({ canActivateLink });
    const reportProgress = vi.fn();
    const { result } = renderHook(() => useNetworkPage('group-1'));

    await expect(
      result.current.handleAcceptRequest([createRelationship()] as never)
    ).rejects.toThrow('Hierarchy member conflict');
    expect(toastErrorMock).toHaveBeenCalledWith('common.network.linkAcceptBlocked');

    canActivateLink.mockReturnValue(true);
    await act(async () => {
      await result.current.handleAcceptRequest([
        createRelationship({ connection_request_id: null }),
      ] as never);
      await result.current.handleRejectRequest([
        createRelationship({ connection_request_id: null }),
      ] as never);
      await result.current.handleAcceptRequest(
        [createRelationship()] as never,
        {
          reportProgress,
        } as never
      );
      await Promise.resolve();
      await result.current.handleAcceptRequest([createRelationship()] as never);
      await Promise.resolve();
    });

    expect(rejectGroupConnectionRequest).not.toHaveBeenCalled();
    expect(reportProgress).toHaveBeenCalledWith({ key: 'commit', status: 'complete' });
    expect(reportProgress).toHaveBeenCalledWith(
      expect.objectContaining({ key: 'sync', status: 'active' })
    );
    expect(toastErrorMock).toHaveBeenCalled();
  });

  it('runs workflow mutation handlers with and without submission progress', async () => {
    const saveWorkflow = vi.fn();
    const approveWorkflowApproval = vi.fn(() => 'approve-workflow');
    const rejectWorkflowApproval = vi.fn(() => 'reject-workflow');
    ...
    trackServerFinalizationMock.mockImplementation((_result, options) => {
      options.onError(new Error('workflow server failure'));
    });
    const reportProgress = vi.fn();
    const { result, rerender } = renderHook(() => useNetworkPage('group-1'));

    await act(async () => {
      await result.current.handleSaveWorkflow({ reportProgress } as never);
      await result.current.handleApproveWorkflowApproval('approval-1', {
        reportProgress,
      } as never);
      await result.current.handleRejectWorkflowApproval('approval-2', {
        reportProgress,
      } as never);
      await result.current.handleApproveWorkflowApproval('approval-3');
      await result.current.handleRejectWorkflowApproval('approval-4');
    });

    expect(saveWorkflow).toHaveBeenCalledWith('user-1', { reportProgress });
    expect(trackServerFinalizationMock).toHaveBeenCalledTimes(2);
    expect(toastErrorMock).toHaveBeenCalled();

    useAuthMock.mockReturnValue({ user: null });
    rerender();
    await act(async () => result.current.handleSaveWorkflow());
    expect(saveWorkflow).toHaveBeenCalledTimes(1);
  });

  it('uses nullable group defaults and ignores workflows without approval rows', () => {
    const nullableGroup = {
      id: 'group-1',
      name: null,
      description: null,
    ...
    const noApprovals = createWorkflow('no-approvals', { approvals: undefined });
    const rejectedOutgoing = createWorkflow('rejected-outgoing', {
    ...
    useGroupDataMock.mockReturnValue({ group: nullableGroup });
    useAllGroupsMock.mockReturnValue({
      groups: [
    ...
    useWorkflowEditorMock.mockReturnValue({
      ...useWorkflowEditorMock(),
      workflows: [noApprovals, rejectedOutgoing],
    });

    const { result, rerender } = renderHook(() => useNetworkPage('group-1'));
    expect(result.current.groupName).toBe('Group');
    expect(result.current.availableGroups).toContainEqual({
      id: 'group-1',
      name: null,
      description: null,
      group_type: 'base',
    ...
    expect(result.current.workflowOutgoingRequests).toEqual([rejectedOutgoing]);

    useAllGroupsMock.mockReturnValue({ groups: [nullableGroup] });
    rerender();
