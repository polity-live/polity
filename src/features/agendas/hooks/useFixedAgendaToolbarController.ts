...
export function useFixedAgendaToolbarController(className?: string) {
  ...
  return {
    className: cn(
      'scrollbar-hide supports-backdrop-blur:bg-background/70 border-border/70 bg-background/95 fixed z-50 min-h-12 justify-between overflow-x-auto border-x-0 border-t-0 border-b px-2 py-1.5 shadow-sm backdrop-blur-md transition-all duration-300',
      getTopOffset(),
      getLeftOffset(),
      getRightOffset(),
      getWidth(),
