...
import {
  ...
  EMPTY_CITY_DESIGN_ADDRESS_VALUES,
  mapCityDesignSelectionAddressToValues,
} from '../logic/cityDesignSelectionAddress';
import {
  getCityDesignMapSelectionCorners,
  ...
  type CityDesignBboxResizeHandle,
...
function isSameGeoPoint(left: CityDesignGeoPoint, right: CityDesignGeoPoint) {
...
export function useStreetAreaPickerController({
  ...
  const persistedAddressValues = useMemo(
    () => mapCityDesignSelectionAddressToValues(selectionAddress),
    [
  ...
  const locationSearchLabels = useMemo<GeoAddressTextMap>(
    () => ({
      country: t('features.amendments.cityDesign.areaPicker.locationFields.country'),
      region: t('features.amendments.cityDesign.areaPicker.locationFields.region'),
      city: t('features.amendments.cityDesign.areaPicker.locationFields.city'),
      post_code: t('features.amendments.cityDesign.areaPicker.locationFields.postCode'),
      street: t('features.amendments.cityDesign.areaPicker.locationFields.street'),
      house_number: t('features.amendments.cityDesign.areaPicker.locationFields.houseNumber'),
    }),
    [t]
  );
  ...
  const locationSearchPlaceholders = useMemo<GeoAddressTextMap>(
    () => ({
      country: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.country'),
      region: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.region'),
      city: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.city'),
      post_code: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.postCode'),
      street: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.street'),
      house_number: t('features.amendments.cityDesign.areaPicker.locationPlaceholders.houseNumber'),
    }),
    [t]
  );
  ...
  const dimensions = useMemo(
    () => getCityDesignMapSelectionDimensions(mapSelection),
    [mapSelection]
  );
  const selectionCorners = useMemo(
    () =>
      getCityDesignMapSelectionCorners(mapSelection).map(
        point => [point.lat, point.lon] as [number, number]
      ),
    [mapSelection]
  );
  const rotateHandlePosition = useMemo(() => {
    const point = getCityDesignMapSelectionRotateHandle(mapSelection);
  ...
  const handleBboxMove = (nextCenter: CityDesignGeoPoint) => {
    reverseRequestIdRef.current += 1;
    userDrivenLocationSearchFieldRef.current = null;
    onMapSelectionChange(moveCityDesignMapSelectionToCenter(mapSelection, nextCenter));
  };

  const handleBboxResize = (handle: CityDesignBboxResizeHandle, point: CityDesignGeoPoint) => {
    onMapSelectionChange(
      resizeCityDesignMapSelectionByHandle({ selection: mapSelection, handle, point })
    );
  };
  ...
  const handleRotationDrag = (point: CityDesignGeoPoint) => {
    onMapSelectionChange(rotateCityDesignMapSelectionToPoint({ selection: mapSelection, point }));
  };
  ...
  const handleLocationResolvedAddress = useCallback(
    (result: GeoResolvedAddress | null, field: GeoAddressField | null) => {
      ...
      if (!isSameGeoPoint(currentSelection.center, nextCenter)) {
        onMapSelectionChange(moveCityDesignMapSelectionToCenter(currentSelection, nextCenter));
  ...
  const handleBboxMoveEnd = useCallback(
    async (nextCenter: CityDesignGeoPoint) => {
      if (readOnly) return;
      ...
      try {
        ...
        const nextAddress = createCityDesignSelectionAddress(result);
        userDrivenLocationSearchFieldRef.current = null;
        setLocationSearchValues(mapCityDesignSelectionAddressToValues(nextAddress));
        setLocationSearchResetKey(key => key + 1);
  ...
  return {
    ...
    onLocationSearchReset: () => {
      reverseRequestIdRef.current += 1;
      userDrivenLocationSearchFieldRef.current = null;
      setLocationSearchValues(EMPTY_CITY_DESIGN_ADDRESS_VALUES);
      setLocationSearchResetKey(key => key + 1);
      onSelectionAddressChange(undefined);
    },
    reactLeafletModule,
