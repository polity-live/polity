...
import { projectGeoPointToLocal } from '../logic/cityDesignProjection';
  ...
  if (loadFailed) {
    return (
      <div className="bg-muted/20 text-muted-foreground flex min-h-[28rem] items-center justify-center p-4 text-sm">
        {t('features.amendments.cityDesign.canvas.loadFailed')}
      </div>
    );
  }
      ...
      >
        ...
        {isLoadingOsm ? (
          <div
            ...
            data-slot="osm-loading-overlay"
          >
            <LoadingProgressBar
              motionStyle="optimistic"
              ariaLabel={t('features.amendments.cityDesign.canvas.loadingOsm')}
        ...
        >
                      ...
                      {section.groups.map(group => (
                        <div key={group.id} className="space-y-1">
                          ...
                          {group.entries.map(entry => (
                            <CityDesignLegendItem key={entry.id} entry={entry} />
                          ))}
                        </div>
                      ))}
                    </div>
          ...
          <CollapsibleTrigger asChild>
            <Button
              ...
              aria-label={t(
                `features.amendments.cityDesign.actions.${legendOpen ? 'collapse' : 'expand'}`,
                {
                  label: t('features.amendments.cityDesign.canvas.legend'),
                }
              )}
              title={t(
                `features.amendments.cityDesign.actions.${legendOpen ? 'collapse' : 'expand'}`,
            ...
            >
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="truncate">
                  {t('features.amendments.cityDesign.canvas.legend')}
                </span>
              </span>
              <ChevronDown
                className={cn(
        ...
        <div className="border-border bg-background/95 absolute bottom-6 left-6 flex flex-wrap items-center gap-3 rounded-md border px-3 py-2 text-xs shadow-lg backdrop-blur">
          <div>
            ...
            <p className="text-muted-foreground">
              {t('features.amendments.cityDesign.canvas.pointsSet', {
      ...
      <div className="space-y-4">
        ...
        <div className="bg-muted/20 rounded-md border p-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <ReadonlyMetric
              label={t('features.amendments.cityDesign.inspector.total')}
              value={
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          {object.provenance?.source === 'osm' ? (
            ...
            >
              ...
              {t('features.amendments.cityDesign.actions.undoOsmImport')}
            </Button>
          ) : null}
          <Button
          ...
          >
            ...
            {t(`features.amendments.cityDesign.actions.${isHidden ? 'show' : 'hide'}`, {
              label: objectLabel,
            })}
          </Button>
          <Button
          ...
          >
            ...
            {t('features.amendments.cityDesign.actions.removeShort')}
          </Button>
        </div>
      </div>
    </div>
...
export function CityDesignOsmPopover({
  ...
  const osmLayer = getCityDesignOsmFeatureLayer(osmWay.kind);
  const relevantTags = getRelevantOsmTags(osmWay.tags);
  const mappedDefinition = osmWay.mappedObjectType
    ? getCityDesignObjectDefinition(osmWay.mappedObjectType)
    : null;
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className="truncate text-base font-semibold">
            {osmWay.label ?? t('features.amendments.cityDesign.inspector.osmFallback')}
          </h2>
        ...
        {mappedDefinition ? (
          <ReadonlyCard
            ...
            value={`${t(mappedDefinition.labelKey)} · ${t(
              `features.amendments.cityDesign.inspector.mappingConfidence.${osmWay.mappingConfidence ?? 'generic'}`
            )}`}
          />
        ) : (
          <ReadonlyCard
            label={t('features.amendments.cityDesign.inspector.mappedAs')}
            value={t('features.amendments.cityDesign.inspector.noSafeMapping')}
          />
        )}
        <ReadonlyCard
          label={t('features.amendments.cityDesign.inspector.points', {
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2">
        {osmWay.mappedObjectType && osmWay.mappingConfidence !== 'generic' ? (
          ...
          >
            ...
            {t('features.amendments.cityDesign.inspector.importAsPlanned')}
          </Button>
        ) : null}
        <Button
        ...
        >
          ...
          {t('features.amendments.cityDesign.inspector.removeFromMap')}
        </Button>
      </div>
    </div>
  );
...
function getCanvasAnchorFromLocalPoint(point: CityDesignLocalPoint): CanvasAnchor {
  return {
...
function getTrackedCanvasAnchorFromLocalPoint({
  ...
  cameraPose: CityDesignCameraPose | null | undefined;
...
export function projectLocalPointToCanvasAnchor(
  point: CityDesignLocalPoint,
  cameraPose: CityDesignCameraPose,
...
function getCanvasAnchorFromOsmWay(
  ...
  const points = getCityDesignOsmFeaturePoints(osmWay);
  if (points.length === 0) return getCanvasAnchorFromLocalPoint({ x: 0, z: 0 });

  const averagePoint = points.reduce<CityDesignGeoPoint>(
    (sum, point) => ({
...
function CityDesignLegendItem({ entry }: { entry: CityDesignLegendEntry }) {
  const { t } = useTranslation();

  return (
    <div className="bg-background/75 flex min-h-10 items-center gap-2 rounded-md border px-2 py-1.5">
      <CityDesignLegendPreview entry={entry} />
...
function getStackedChangeRequestMarkers(
  markers: ReturnType<typeof getCityDesignChangeRequestMarker>[]
) {
...
function CityDesignLegendPreview({ entry }: { entry: CityDesignLegendEntry }) {
  ...
  return (
    <span
      ...
      data-testid={`city-design-legend-preview-${previewKind}`}
...
function getLegendPreviewKind(entry: CityDesignLegendEntry) {
  if (entry.objectType) return entry.objectType;
  if (entry.layer) return entry.layer;
  return entry.renderKind;
}

function renderLegendPreviewContent(entry: CityDesignLegendEntry, previewKind: string) {
  const color = entry.color;

  switch (previewKind as CityDesignObjectType | string) {
    case 'tree': {
...
function getLegendTreeSpecies(entry: CityDesignLegendEntry) {
...
function getLegendStringProperty(entry: CityDesignLegendEntry, key: string, fallback: string) {
