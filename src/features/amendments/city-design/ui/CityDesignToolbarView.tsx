...
interface CityDesignToolbarViewProps {
...
} satisfies Record<CityDesignObjectType, ComponentType<{ className?: string }>>;
...
export function CityDesignToolbarView({
  selectedTool,
  ...
  const [openAddedCategories, setOpenAddedCategories] = useState<CityDesignObjectCategory[]>([]);
  ...
  const setAddedCategoryOpen = (category: CityDesignObjectCategory, open: boolean) => {
    ...
    <aside className="bg-background/95 flex h-full min-w-0 flex-col gap-5 border-b p-4 shadow-sm xl:border-r xl:border-b-0">
      <div>
        <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
          ...
          {t('features.amendments.cityDesign.toolbar.mode')}
        </div>
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            ...
            {t('features.amendments.cityDesign.modes.camera')}
          </Button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm font-semibold">
              ...
              {t('features.amendments.cityDesign.toolbar.existing.title')}
            </div>
            <p className="text-muted-foreground mt-0.5 text-[11px] leading-tight">
              {t('features.amendments.cityDesign.toolbar.existing.description')}
            </p>
          </div>
          ...
          <div className="mt-3 space-y-2">
            ...
            <Button
              ...
              title={getActionLabel(
                ...
                t('features.amendments.cityDesign.osmLayers.streetMarkings')
              )}
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm font-semibold">
              ...
              {t('features.amendments.cityDesign.toolbar.elements.title')}
            </div>
            <p className="text-muted-foreground mt-0.5 text-[11px] leading-tight">
              {t('features.amendments.cityDesign.toolbar.elements.description')}
            </p>
          </div>
          <CollapsibleTrigger asChild>
            <Button
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <Layers className="text-muted-foreground size-4" />
              {t('features.amendments.cityDesign.toolbar.added.title')}
            </div>
            <p className="text-muted-foreground mt-0.5 text-[11px] leading-tight">
              {t('features.amendments.cityDesign.toolbar.added.description')}
            </p>
          </div>
          <CollapsibleTrigger asChild>
            <Button
          ...
          <div className="mt-3 space-y-3">
            {addedObjectGroups.length === 0 ? (
              <div className="bg-background/80 text-muted-foreground rounded-md border px-3 py-3 text-xs">
                {t('features.amendments.cityDesign.toolbar.added.empty')}
              </div>
            ) : (
                    ...
                    <CollapsibleContent>
                      <div className="space-y-1.5 pl-3">
                        {group.objects.map(object => {
                          const definition = cityDesignObjectRegistry[object.type];
                          ...
                          const objectLabel = t(
                            getCityDesignObjectVariantLabelKey(object) ?? definition.labelKey
                          );
