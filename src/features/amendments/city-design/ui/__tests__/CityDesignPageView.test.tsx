...
vi.mock('../CityDesignTopBarView', () => ({
  CityDesignTopBarView: ({
    ...
    <div data-testid="city-design-topbar">
...
describe('CityDesignPageView', () => {
  it('relies on the shared entity frame without another horizontal workspace inset', () => {
    const { container } = render(<CityDesignPageView {...createPageProps()} />);
    const page = container.firstElementChild;
    const workspace = container.querySelector('[data-slot="city-design-page-content"]');

    expect(page?.className).not.toContain('pt-5');
    expect(page?.getAttribute('data-tutorial-anchor')).toBe('city-design-tree-placement-workspace');
  ...
  it('hides both action bars for a signed-out read-only viewer', () => {
    ...
    expect(screen.queryByTestId('city-design-topbar')).toBeNull();
  ...
  it('closes the area picker dialog when loading OSM from the picker', () => {
    ...
    render(<CityDesignPageView {...props} />);
...
function createPageProps(overrides: Partial<Parameters<typeof CityDesignPageView>[0]> = {}) {
  const design = {
    ...createEmptyCityDesignState(),
    osmSnapshot: {
      fetchedAt: 0,
  ...
  return {
    ...
    ...overrides,
  } as Parameters<typeof CityDesignPageView>[0];
}
