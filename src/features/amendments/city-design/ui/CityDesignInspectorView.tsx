...
import { formatMinorCurrency, getCityDesignCostCatalogEntry } from '../logic/cityDesignCostCatalog';
...
import {
  getCityDesignObjectVariantLabelKey,
  getCityDesignVariantLabelKey,
} from '../logic/cityDesignVariantCatalog';

interface CityDesignInspectorViewProps {
  ...
  readOnly: boolean;
  currency?: string;
  onPlacementPropertyChange: (key: string, value: CityDesignPropertyValue) => void;
...
function asInputValue(value: CityDesignPropertyValue | undefined) {
...
function getPlacementTotalMinor(args: {
  ...
  const definition = getCityDesignObjectDefinition(args.placementSettings.type);
  const catalogEntry = getCityDesignCostCatalogEntry(definition.type, args.currency);
...
function getPlacementRotationValue(
  ...
  return getCityDesignGeometryRotationDeg(placementPreview);
}

function getOsmLayerLabelKey(layer: ReturnType<typeof getCityDesignOsmFeatureLayer>) {
  if (layer === 'bike_lane') return 'features.amendments.cityDesign.osmLayers.bikeLane';
  if (layer === 'street_furniture')
    return 'features.amendments.cityDesign.osmLayers.streetFurniture';
  if (layer === 'landuse_context') return 'features.amendments.cityDesign.osmLayers.landuseContext';
  return `features.amendments.cityDesign.osmLayers.${layer}`;
}
...
export function CityDesignInspectorView({
  ...
  onDeleteObject,
  onHideOsmWay,
}: CityDesignInspectorViewProps) {
  const { t } = useTranslation();
  const fieldLabel = (labelKey: string, unit?: string) =>
    unit
      ? t('features.amendments.cityDesign.inspector.fieldWithUnit', {
          label: t(labelKey),
          unit,
        })
      : t(labelKey);
  ...
  if (interactionMode === 'place') {
    const definition = getCityDesignObjectDefinition(selectedTool);
    const objectLabel = t(
      getCityDesignVariantLabelKey(selectedTool, placementSettings.properties) ??
        definition.labelKey
    );
    const hasWidth = definition.toolMode !== 'point' || definition.defaultWidth != null;
    const catalogEntry = getCityDesignCostCatalogEntry(selectedTool, currency);
      ...
      <aside className="bg-background/95 min-w-0 border-b p-4 shadow-sm xl:border-b-0 xl:border-l">
        <div className="mb-4">
          <p className="text-muted-foreground mb-1 flex items-center gap-2 text-xs font-medium uppercase">
            ...
            {t('features.amendments.cityDesign.inspector.place')}
          </p>
          <h2 className="text-base font-semibold">{objectLabel}</h2>
          <p className="text-muted-foreground text-xs">
            {placementMode
              ? t('features.amendments.cityDesign.inspector.activeDraft')
              : t('features.amendments.cityDesign.inspector.nextElementSettings')}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-muted/20 grid grid-cols-2 gap-2 rounded-md border p-2">
            ...
            {placementPreview ? (
              <>
                ...
                <div className="space-y-1">
                  <Label className="text-xs">
                    {t('features.amendments.cityDesign.inspector.area')}
                  </Label>
          ...
          {definition.propertySchema.map(field => {
            ...
            if (field.fieldType === 'combobox') {
              const datalistId = `city-design-placement-${field.key}`;
          ...
          <div className="border-border bg-muted/20 mt-4 rounded-md border p-3">
            ...
            <p className="text-muted-foreground mt-2 text-xs">
              {t('features.amendments.cityDesign.inspector.suggestedCost', {
  ...
  if (!selectedObject) {
    if (selectedOsmWay) {
      const osmFeaturePoints = getCityDesignOsmFeaturePoints(selectedOsmWay);
      const osmLayer = getCityDesignOsmFeatureLayer(selectedOsmWay.kind);
      ...
      const osmElevationDetails = [
        selectedOsmWay.structureKind
          ? `${t('features.amendments.cityDesign.inspector.structure')}: ${
              selectedOsmWay.structureKind
            }`
          : null,
        selectedOsmWay.elevationSource
          ? `${t('features.amendments.cityDesign.inspector.elevationSource')}: ${
              selectedOsmWay.elevationSource
            }`
          : null,
        selectedOsmWay.incline
          ? `${t('features.amendments.cityDesign.inspector.incline')}: ${selectedOsmWay.incline}`
          : null,
        isFiniteNumber(selectedOsmWay.stepCount)
          ? `${t('features.amendments.cityDesign.inspector.stepCount', {
              count: selectedOsmWay.stepCount,
            })}: ${selectedOsmWay.stepCount}`
          : null,
        isFiniteNumber(selectedOsmWay.clearanceMeters)
          ? `${t('features.amendments.cityDesign.inspector.clearance')}: ${formatMeters(
        ...
        <aside className="bg-background/95 min-w-0 border-b p-4 shadow-sm xl:border-b-0 xl:border-l">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              <p className="text-muted-foreground mb-1 flex items-center gap-2 text-xs font-medium uppercase">
                ...
                {t('features.amendments.cityDesign.inspector.existing')}
              </p>
              <h2 className="text-base font-semibold">
                {selectedOsmWay.label ?? t('features.amendments.cityDesign.inspector.osmFallback')}
              </h2>
              <p className="text-muted-foreground text-xs">
                {osmKindLabel}
            ...
            <Button
              ...
              size="icon"
              variant="outline"
              title={t('features.amendments.cityDesign.inspector.removeFromMap')}
              disabled={readOnly}
          ...
          <div className="grid gap-2 text-sm">
            <div className="bg-muted/20 rounded-md border px-3 py-2">
              <p className="text-muted-foreground text-xs">
                {t('features.amendments.cityDesign.inspector.points', {
            ...
            {selectedOsmWay.height ? (
              <div className="bg-muted/20 rounded-md border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  {t('features.amendments.cityDesign.inspector.height')}
                </p>
            ...
            {isFiniteNumber(selectedOsmWay.deckElevationMeters) ? (
              <div className="bg-muted/20 rounded-md border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  {t('features.amendments.cityDesign.inspector.deckElevation')}
                </p>
            ...
            selectedOsmWay.baseElevationMeters !== 0 ? (
              <div className="bg-muted/20 rounded-md border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  {t('features.amendments.cityDesign.inspector.baseElevation')}
                </p>
            ...
            {isFiniteNumber(selectedOsmWay.layerIndex) && selectedOsmWay.layerIndex !== 0 ? (
              <div className="bg-muted/20 rounded-md border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  {t('features.amendments.cityDesign.inspector.layer')}
                </p>
            ...
            {selectedOsmWay.semanticUse || selectedOsmWay.level || selectedOsmWay.access ? (
              <div className="bg-muted/20 rounded-md border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  {t('features.amendments.cityDesign.inspector.tags')}
                </p>
      ...
      <aside className="bg-background/95 min-w-0 border-b p-4 shadow-sm xl:border-b-0 xl:border-l">
        <div className="bg-muted/20 rounded-md border border-dashed p-4">
          <p className="text-muted-foreground mb-2 flex items-center gap-2 text-xs font-medium uppercase">
            <SlidersHorizontal className="size-3.5" />
            {t('features.amendments.cityDesign.inspector.title')}
          </p>
          <h2 className="text-base font-semibold">
            {t('features.amendments.cityDesign.inspector.noSelection')}
          </h2>
          <p className="text-muted-foreground mt-2 text-sm">
            {t('features.amendments.cityDesign.inspector.noSelectionDescription')}
          </p>
        </div>
      </aside>
    );
  ...
  const definition = getCityDesignObjectDefinition(selectedObject.type);
  const objectLabel = t(getCityDesignObjectVariantLabelKey(selectedObject) ?? definition.labelKey);
  const unitCostMajor = minorToMajor(
    ...
    <aside className="bg-background/95 min-w-0 border-b p-4 shadow-sm xl:border-b-0 xl:border-l">
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <Button
          ...
          size="icon"
          variant="outline"
          title={t('features.amendments.cityDesign.actions.remove', { label: objectLabel })}
          disabled={readOnly}
      ...
      <div className="space-y-4">
          ...
          <div className="bg-muted/20 grid grid-cols-2 gap-2 rounded-md border p-2">
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                step={1}
                value={Number(getCityDesignGeometryRotationDeg(selectedObject.geometry).toFixed(1))}
                disabled={readOnly}
        ...
        {definition.propertySchema.map(field => {
          ...
          if (field.fieldType === 'combobox') {
            const datalistId = `city-design-object-${selectedObject.id}-${field.key}`;
