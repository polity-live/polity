...
type CityDesignAmendmentSummary =
  | {
      title?: string | null;
...
interface CityDesignPageViewProps {
  amendmentId: string;
  amendment: CityDesignAmendmentSummary;
  isLoading: boolean;
  ...
  streetChangeRequests: readonly CityDesignChangeRequest[];
  ...
  selectedCenter: CityDesignGeoPoint;
  selectedBbox: CityDesignBoundingBox;
  selectedMapSelection: CityDesignMapSelection;
  ...
  onSelectedMapSelectionChange: (selection: CityDesignMapSelection) => void;
  ...
  onComparisonModeChange: (mode: CityDesignStateV1['comparisonMode']) => void;
  onScenePointerDown: (point: CityDesignLocalPoint) => void;
  onScenePointerMove: (point: CityDesignLocalPoint) => void;
  onScenePointerHover: (
    point: CityDesignLocalPoint | null,
  ...
  onDeleteObjectCategory: (category: CityDesignObjectCategory) => void;
}

export function CityDesignPageView({
  amendmentId,
  amendment,
  isLoading,
  showActionBars,
  ...
  const [pendingCategoryDeletion, setPendingCategoryDeletion] =
    useState<CityDesignObjectCategory | null>(null);
  const osmWayCount = useMemo(
    () => getCityDesignOsmFeatures(design.osmSnapshot).length,
    [design.osmSnapshot]
  );
  const title = amendment?.title ?? t('features.amendments.cityDesign.defaultTitle');
  const pendingCategoryDeletionCount = pendingCategoryDeletion
    ? design.objects.filter(
        object => getCityDesignObjectDefinition(object.type).category === pendingCategoryDeletion
      ).length
    : 0;
  ...
  const areaPickerContent = useMemo(
    () => (
      <StreetAreaPicker
        ...
        variant="panel"
        tutorialActive={Boolean(amendment?.tutorial_run_id)}
      ...
      {showActionBars ? (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {t('features.amendments.cityDesign.categoryDelete.title')}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {t('features.amendments.cityDesign.categoryDelete.description', {
...
function CityDesignNavigationHelp() {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const helpTabs = [
    {
      key: 'touch',
      label: t('features.amendments.cityDesign.help.tabs.touch'),
      sections: [
        {
          key: 'select',
          title: t('features.amendments.cityDesign.help.modes.select'),
          items: [t('features.amendments.cityDesign.help.touch.select')],
        },
        {
          key: 'place',
          title: t('features.amendments.cityDesign.help.modes.place'),
          items: [t('features.amendments.cityDesign.help.touch.place')],
        },
        {
          key: 'camera',
          title: t('features.amendments.cityDesign.help.modes.camera'),
    ...
    {
      key: 'mouse',
      label: t('features.amendments.cityDesign.help.tabs.mouse'),
      sections: [
        {
          key: 'select',
          title: t('features.amendments.cityDesign.help.modes.select'),
          items: [
            t('features.amendments.cityDesign.help.mouse.select'),
            t('features.amendments.cityDesign.help.mouse.rotate'),
          ],
        },
        {
          key: 'place',
          title: t('features.amendments.cityDesign.help.modes.place'),
          items: [t('features.amendments.cityDesign.help.mouse.place')],
        },
        {
          key: 'camera',
          title: t('features.amendments.cityDesign.help.modes.camera'),
    ...
    {
      key: 'keyboard',
      label: t('features.amendments.cityDesign.help.tabs.keyboard'),
      sections: [
        {
          key: 'select',
          title: t('features.amendments.cityDesign.help.modes.select'),
          items: [t('features.amendments.cityDesign.help.keyboard.select')],
        },
        {
          key: 'place',
          title: t('features.amendments.cityDesign.help.modes.place'),
          items: [t('features.amendments.cityDesign.help.keyboard.place')],
        },
        {
          key: 'camera',
          title: t('features.amendments.cityDesign.help.modes.camera'),
      ...
      <PopoverContent align="end" className="w-[min(34rem,calc(100vw-2rem))] p-0">
        <div className="space-y-4 p-4 text-sm">
          <div className="space-y-1">
            <h3 className="font-semibold">{t('features.amendments.cityDesign.help.title')}</h3>
            <p className="text-muted-foreground text-xs">
              {t('features.amendments.cityDesign.help.description')}
            </p>
          </div>
            ...
            {helpTabs.map(tab => (
              <TabsContent key={tab.key} value={tab.key} className="space-y-3">
                ...
                <section className="bg-muted/20 rounded-md border p-3">
                  <h4 className="text-muted-foreground text-xs font-semibold uppercase">
                    {t('features.amendments.cityDesign.help.global.title')}
                  </h4>
