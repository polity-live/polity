...
interface CityDesignWorkspaceViewProps {
  topBar?: ReactNode;
...
export function CityDesignWorkspaceView({
  topBar,
  beforeCard,
  title,
        ...
        <div className="flex min-w-0 flex-wrap items-center justify-end gap-2 text-xs">
          ...
          {isDirty ? (
            <BadgeControl
              ...
              shape="rounded"
              role="status"
              aria-label={t('features.amendments.cityDesign.status.unsavedChanges')}
              className="h-9 gap-1.5 px-3 py-2 font-medium whitespace-nowrap"
            >
              <AlertTriangle className="size-3.5" aria-hidden="true" />
              {t('features.amendments.cityDesign.status.unsavedChanges')}
            </BadgeControl>
          ) : null}
          {headerActions}
        </div>
  ...
  return (
    <div
      ...
      data-testid="city-design-workspace"
