import type {
  ...
  CityDesignOsmMappingConfidence,
  CityDesignOsmRenderProfile,
  CityDesignPropertyValue,
} from '../types';

export interface CityDesignOsmSemanticMapping {
  objectType: CityDesignObjectType | null;
...
function compactProperties(
  properties: Record<string, CityDesignPropertyValue | undefined>
): Record<string, CityDesignPropertyValue> {
  return Object.fromEntries(
    Object.entries(properties).filter(
      (entry): entry is [string, CityDesignPropertyValue] => entry[1] !== undefined
    )
  );
}
...
export function getCityDesignOsmRoadWidthMeters(tags: Record<string, string>) {
...
export function getCityDesignOsmSideWidthMeters(args: {
  tags: Record<string, string>;
...
export function getCityDesignOsmSemanticMapping(
  ...
  >
): CityDesignOsmSemanticMapping {
  const tags = feature.tags ?? {};
  const confidence: CityDesignOsmMappingConfidence =
  ...
  switch (feature.kind) {
    ...
    case 'green': {
      const type: CityDesignObjectType =
...
export function applyCityDesignOsmSemanticMapping(
  feature: CityDesignOsmFeature
): CityDesignOsmFeature {
  const mapping = getCityDesignOsmSemanticMapping(feature);
  return {
    ...feature,
