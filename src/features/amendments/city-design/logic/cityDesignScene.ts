...
import { getCityDesignComparisonLayers } from './cityDesignDiff';
import { CITY_DESIGN_CURRENCY, getCityDesignObjectDefinition } from './cityDesignObjectRegistry';
import {
...
import {
  ...
  type CityDesignChangeRequestOverlayObject,
  type CityDesignChangeRequestTone,
} from './cityDesignChangeRequests';
import {
  type CityDesignInputAction,
...
export interface CityDesignSceneMountOptions {
  ...
  readOnly: boolean;
  initialCameraPose: CityDesignCameraPose | null;
...
export interface CityDesignSceneController {
  updateDesign: (
    options: Pick<
...
const CITY_DESIGN_SPLIT_LAYER_OFFSET_X = 52;

export function getCityDesignComparisonLayerOffsetX(
  ...
  return layer === 'original'
    ? -CITY_DESIGN_SPLIT_LAYER_OFFSET_X
    : CITY_DESIGN_SPLIT_LAYER_OFFSET_X;
}

export function getCityDesignPointerLayer(
  comparisonMode: CityDesignComparisonMode,
  worldX: number
): CityDesignComparisonLayer {
  const layers = getCityDesignComparisonLayers(comparisonMode);
  if (layers.split) return worldX < 0 ? 'original' : 'design';
  return layers.showDesign ? 'design' : 'original';
}

export function normalizeCityDesignPointerPoint(
  point: CityDesignLocalPoint,
  comparisonMode: CityDesignComparisonMode,
  layer: CityDesignComparisonLayer
): CityDesignLocalPoint {
  return {
    x: point.x - getCityDesignComparisonLayerOffsetX(comparisonMode, layer),
    z: point.z,
  };
}

export interface CityDesignRenderFrameApi {
...
export function createCityDesignRenderScheduler(
  renderFrame: FrameRequestCallback,
  frameApi: CityDesignRenderFrameApi = window
) {
...
export function toShapePoint(point: CityDesignLocalPoint) {
  return {
    x: point.x,
    y: -point.z,
  };
...
function toGroundVector(THREE: ThreeModule, point: CityDesignLocalPoint, y: number) {
  return new THREE.Vector3(point.x, y, point.z);
}

function makeShape(THREE: ThreeModule, points: CityDesignLocalPoint[]) {
...
function normalizeDirection(start: CityDesignLocalPoint, end: CityDesignLocalPoint) {
...
function offsetPointFromDirection(
  point: CityDesignLocalPoint,
  direction: CityDesignLocalPoint,
  offset: number
) {
  return {
...
function getCenterlineSample(centerline: CityDesignLocalPoint[], distance: number) {
...
export function getCityDesignOsmFeatureRenderY(feature: CityDesignOsmWay, fallbackY = 0.05) {
...
export function getCityDesignOsmWaterRenderY(feature: CityDesignOsmWay) {
...
function isElevatedOsmStructure(feature: CityDesignOsmWay) {
  return (
...
export interface CityDesignElevationRampFeature {
  id: string;
...
export interface CityDesignElevationRampSegment {
  sourceId: string;
...
function distanceBetweenLocalPoints(start: CityDesignLocalPoint, end: CityDesignLocalPoint) {
  return Math.hypot(end.x - start.x, end.z - start.z);
}

function createCorridorGeometryFromCenterline(points: CityDesignLocalPoint[], width: number) {
...
function canFeatureReceiveElevationRamp(kind: CityDesignOsmFeatureKind) {
  return (
    kind === 'road' ||
    kind === 'rail' ||
    kind === 'bike_lane' ||
...
function canFeatureStartElevationRamp(feature: CityDesignElevationRampFeature) {
  return feature.surfaceY > 0.8 && feature.structureKind === 'bridge';
}

export function getCityDesignElevationRampLength(deckElevationMeters: number, fallback = false) {
...
function getCityDesignElevationRampConnections(
  source: CityDesignElevationRampFeature,
  endpoint: ElevationRampEndpoint,
  ...
  return features
    ...
    .filter(
      (
        ...
        feature: CityDesignElevationRampFeature;
        endpoint: ElevationRampEndpoint;
        point: CityDesignLocalPoint;
      } =>
...
function getInternalElevationRampLength(
  ...
  return Math.min(getCityDesignElevationRampLength(feature.surfaceY), maxRampLength);
}

export function getCityDesignElevationRampSegments(
  features: CityDesignElevationRampFeature[]
): CityDesignElevationRampSegment[] {
  return features.flatMap(feature => {
    ...
    const candidates = (['start', 'end'] as const).flatMap(endpoint => {
      ...
      const connections = getCityDesignElevationRampConnections(feature, endpoint, features);
...
function getCityDesignManualElevationRampSegments(args: {
  id: string;
  kind: CityDesignOsmFeatureKind;
  geometry: RenderableCorridorGeometry;
  surfaceY: number;
  ...
  const rampLength = Math.min(
    getCityDesignElevationRampLength(args.surfaceY),
...
export type CityDesignTreeRenderKind =
  'deciduous' | 'conifer' | 'fruit' | 'columnar_poplar' | 'ornamental_cherry' | 'flowering_plum';

export type CityDesignTreeCanopyShape = 'rounded_lobes' | 'stacked_cones' | 'columnar_lobes';

export interface CityDesignTreeRenderProfile {
  canopyColors?: string[];
  canopyShape: CityDesignTreeCanopyShape;
  hasFruitMarkers: boolean;
  kind: CityDesignTreeRenderKind;
  trunkHeightRatio: number;
}

export function getCityDesignTreeRenderKind(value: unknown): CityDesignTreeRenderKind {
...
export function getCityDesignTreeRenderProfile(value: unknown): CityDesignTreeRenderProfile {
  const kind = getCityDesignTreeRenderKind(value);
  if (kind === 'conifer') {
    return {
...
function addPointObject(args: {
  ...
  if (definition.renderKind === 'tree') {
    ...
    const treeRenderProfile = getCityDesignTreeRenderProfile(object.properties.species);
...
function getDesignObjectSurfaceY(object: CityDesignObject, fallbackY: number) {
  const deckElevation = numberProperty(object.properties.deckElevationMeters, fallbackY);
  if (deckElevation <= 0) return fallbackY;
  return deckElevation;
}

function getDesignObjectStructureKind(object: CityDesignObject) {
  return stringProperty(object.properties.structureKind, '');
}

function getDesignObjectElevationRampKind(
  object: CityDesignObject,
  definition: ReturnType<typeof getCityDesignObjectDefinition>
): CityDesignOsmFeatureKind | null {
...
function getObjectCenter(object: CityDesignObject) {
  return getCityDesignGeometryCenter(object.geometry);
}

function getObjectRadius(object: CityDesignObject) {
...
function getObjectFocusPoint(design: CityDesignStateV1, objectId: string | null) {
...
function isObjectVisible(
  ...
  hiddenObjectCategories: Set<CityDesignObjectCategory>
) {
  if (hiddenObjectIds.has(object.id)) return false;
  return !hiddenObjectCategories.has(getCityDesignObjectDefinition(object.type).category);
}

function offsetOsmLocalPoints(points: CityDesignLocalPoint[], offsetMeters: number | undefined) {
...
function getOsmWayLocalPoints(way: CityDesignOsmWay, design: CityDesignStateV1) {
  const localPoints = getCityDesignOsmFeaturePoints(way).map(point =>
...
function getOsmWayFocusPoint(design: CityDesignStateV1, osmWayId: string | null) {
  if (!osmWayId || !design.osmSnapshot) return null;
  const way = getCityDesignOsmFeatures(design.osmSnapshot).find(item => item.id === osmWayId);
  if (!way) return null;
...
function addWaterSurface(args: {
  THREE: ThreeModule;
  group: Group;
  geometry: RenderableCorridorGeometry | { polygon: CityDesignLocalPoint[] };
  color?: string;
  opacity?: number;
  y?: number;
  objectId?: string;
...
function getCenterlineDistances(centerline: CityDesignLocalPoint[]) {
  const distances = [0];
...
function getRampSurfaceColor(kind: CityDesignOsmFeatureKind, fallbackColor: string) {
...
function addElevationRampSegment(args: {
  THREE: ThreeModule;
  group: Group;
  segment: CityDesignElevationRampSegment;
  color: string;
}) {
...
function addInternalBridgeDeck(args: {
  ...
  objectId?: string;
  osmWayId?: string;
  rampSegments: CityDesignElevationRampSegment[] | undefined;
}) {
...
function addDesignObject(args: {
...

  {
    ...
    const elevationRampSegments = rampKind
      ? getCityDesignManualElevationRampSegments({
          id: object.id,
          kind: rampKind,
          geometry: object.geometry,
...
function getChangeRequestOverlayColor(tone: Exclude<CityDesignChangeRequestTone, 'neutral'>) {
...
function addChangeRequestOverlayObject(args: {
  THREE: ThreeModule;
  group: Group;
  overlay: CityDesignChangeRequestOverlayObject;
  selected: boolean;
  colorMode: CityDesignChangeRequestColorMode;
}) {
...
function createOsmTreeObject(args: {
  ...
  const definition = getCityDesignObjectDefinition('tree');
...
function getOsmLineFeatureSurfaceY(way: CityDesignOsmWay) {
  if (way.kind === 'rail') {
    return getCityDesignOsmFeatureRenderY(way, way.level === 'tunnel' ? 0.035 : 0.07);
  }
  if (way.kind === 'road') {
    return getCityDesignOsmFeatureRenderY(way, way.level === 'tunnel' ? 0.035 : 0.052);
  }
  if (way.kind === 'parking') {
...
function buildOsmElevationRampFeature(args: {
  way: CityDesignOsmWay;
  localPoints: CityDesignLocalPoint[];
}): CityDesignElevationRampFeature | null {
  ...
  visibleOsmFeatures.forEach(way => {
    ...
    if (way.kind === 'sidewalk' || way.kind === 'bike_lane' || way.kind === 'parking') {
      ...
      const surfaceY = isSteps
        ? 0.058
        : getCityDesignOsmFeatureRenderY(way, isParking ? 0.06 : 0.058);
      const elevationRampSegments = elevationRampsByFeatureId.get(way.id);
      const flatGeometry =
      ...
      if (isSteps) {
        addStairTreads({
          ...
          osmWayId: way.id,
          y: surfaceY + 0.066,
          endY: getCityDesignOsmFeatureRenderY(way, 0.8),
...
function addRotateHandle(args: { THREE: ThreeModule; group: Group; object: CityDesignObject }) {
  const { THREE, group, object } = args;
  const center = getObjectCenter(object);
  const radius = getObjectRadius(object) + 2.4;
  const rotationRad = (getCityDesignGeometryRotationDeg(object.geometry) * Math.PI) / 180;
  const handlePoint = {
...
export async function mountCityDesignScene(
  initialOptions: CityDesignSceneMountOptions
): Promise<CityDesignSceneController> {
  ...
  let layers = getCityDesignComparisonLayers(options.design.comparisonMode);
  const {
    originalGroup,
    designGroup,
    originalLayerGroup,
  ...
  function mergeSceneOptions(partialOptions: Partial<CityDesignSceneMountOptions>) {
  ...
  function rebuildChangeRequestLayer() {
    ...
    if (layers.showDesign) {
      getCityDesignChangeRequestOverlayObjects(options.changeRequests).forEach(overlay => {
  ...
  function hasOsmRenderInputsChanged(
    previousOptions: CityDesignSceneMountOptions,
    nextOptions: CityDesignSceneMountOptions
  ) {
    return (
  ...
  function setPrimaryPointerNavigationControls(action: CityDesignInputAction) {
    controls.mouseButtons.LEFT =
  ...
  function toDesignPoint(point: CityDesignLocalPoint) {
    const designPoint = {
  ...
  function getPointerLayer(point: CityDesignLocalPoint): CityDesignComparisonLayer {
    return getCityDesignPointerLayer(options.design.comparisonMode, point.x);
  }
  ...
  function toLayerPoint(
    ...
    return normalizeCityDesignPointerPoint(point, options.design.comparisonMode, layer);
  }
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    const action = getCityDesignKeyboardAction(event.key);
