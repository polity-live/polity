...
import { buildCityDesignLegendSections } from '../cityDesignLegend';
import { DEFAULT_CITY_DESIGN_OSM_LAYER_VISIBILITY } from '../cityDesignOsm';

describe('buildCityDesignLegendSections', () => {
  it('builds placement preset legend entries with stable preset ids', () => {
    const sections = buildCityDesignLegendSections({
    ...
    expect(planned?.labelKey).toBe('features.amendments.cityDesign.canvas.legendPlanned');
    ...
    expect(labelKeys).toContain('features.amendments.cityDesign.variantLabels.building.office');
    expect(labelKeys).toContain(
      'features.amendments.cityDesign.variantLabels.building.residential'
    );
  ...
  it('keeps existing OSM layers occurrence-based, visible, and deduplicated', () => {
    ...
    expect(existing?.entries[0]?.labelKey).toBe(
      'features.amendments.cityDesign.objects.street.label'
    );
  });
});
