...
import {
  ...
  getStreetDesignOsmSemanticMapping,
  getStreetDesignOsmSideWidthMeters,
} from '../streetDesignOsmMapping';

describe('streetDesignOsmMapping', () => {
  it('maps lane metadata and explicit carriageway width to a street', () => {
    ...
    const mapping = getStreetDesignOsmSemanticMapping({
      kind: 'road',
      geometryKind: 'line',
      tags,
      source: 'osm',
    ...
    expect(getStreetDesignOsmRoadWidthMeters(tags)).toBe(10.5);
  ...
  it('derives side widths from detailed sidewalk, cycleway, and parking tags', () => {
    ...
    expect(getStreetDesignOsmSideWidthMeters({ tags, kind: 'sidewalk', side: 'right' })).toBe(3);
    expect(getStreetDesignOsmSideWidthMeters({ tags, kind: 'bike_lane', side: 'right' })).toBe(2.6);
    expect(getStreetDesignOsmSideWidthMeters({ tags, kind: 'parking', side: 'right' })).toBe(4.8);
  });
