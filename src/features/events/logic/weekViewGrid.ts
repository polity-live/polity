...
type WeekGridEvent = Pick<CalendarEvent, 'id' | 'start_date' | 'end_date'>;
...
export function getWeekEventRange(event: WeekGridEvent): {
  ...
  const rawEndMinute =
    typeof event.end_date === 'number' && event.end_date > 0
...
export function buildDayTimeLayout<T>(
  items: readonly T[],
  getStartTimestamp: (item: T) => number,
  getEndTimestamp: (item: T) => number | null | undefined
): DayTimeLayout<T>[] {
...
export function buildWeekEventLayout<T extends WeekGridEvent>(
  ...
  weekDays.forEach((day, dayIndex) => {
    ...
    const dayLayout = buildDayTimeLayout(
      dayEvents,
