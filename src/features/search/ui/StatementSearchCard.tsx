...
export function StatementSearchCard({ item }: StatementSearchCardProps) {
  ...
  return (
    <article className="bg-card hover:bg-accent/50 relative rounded-lg border transition-colors">
      <Link
        to="/statement/$id"
        params={{ id: item.id }}
        data-action-id="search.statement.open"
      ...
      >
        <div className="flex gap-3 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-muted-foreground mt-2 flex flex-wrap items-center gap-2 pr-7 text-xs">
              {/* Creator */}
      ...
      <ShareButton
        ...
        variant="ghost"
        size="sm"
        className="absolute right-3 bottom-3 z-10 h-5 w-5 p-0"
      />
    </article>
  );
}
