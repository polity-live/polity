...
function createTx(results: unknown[] = [], location: 'client' | 'server' = 'client') {
  ...
  return {
    ...
    mutate: {
      election: ops(),
      election_candidate: ops(),
      elector: ops(),
      indicative_elector_participation: ops(),
      indicative_candidate_selection: ops(),
      final_elector_participation: ops(),
      final_candidate_selection: ops(),
...
describe('election shared mutator branch contracts', () => {
  it('runs optimistic CRUD, candidacy, elector, and selection facades on the client', async () => {
    ...
    await electionSharedMutators.updateElection.fn({
      tx: tx as never,
      ctx,
      args: { id: 'election-1' } as never,
    });
    await electionSharedMutators.updateElection.fn({
      tx: tx as never,
      ctx,
      args: { id: 'election-1', election_mode: 'list' } as never,
    });
    await electionSharedMutators.updateElection.fn({
      tx: tx as never,
      ctx,
      args: { id: 'election-1', seat_count: 3 } as never,
    });
    await electionSharedMutators.updateElection.fn({
      tx: tx as never,
      ctx,
      args: { id: 'election-1', max_votes: 2, position_id: 'role-2' } as never,
    });
    ...
    await electionSharedMutators.updateCandidate.fn({
      tx: tx as never,
      ctx,
      args: { id: 'candidate-1', name: 'Ada Updated' } as never,
    });
  ...
  it('validates manager scopes, missing elections, and RBAC fallback', async () => {
    ...
    expect(canMock).toHaveBeenLastCalledWith(fallback, ctx, {
      action: 'manage_votes',
      resource: 'events',
      eventId: 'event-1',
    });
    ...
    await expect(
      electionSharedMutators.deleteElection.fn({
        tx: createTx([{ id: 'election-2', agenda_item_id: null }], 'server') as never,
        ctx,
        args: { id: 'election-2' },
      })
    ).rejects.toBeInstanceOf(PermissionError);
    ...
    await expect(
      electionSharedMutators.deleteElection.fn({
        tx: createTx([{ id: 'election-3', agenda_item_id: 'missing' }, null], 'server') as never,
        ctx,
        args: { id: 'election-3' },
      })
    ).rejects.toBeInstanceOf(PermissionError);
  ...
  it('validates server candidate and frozen-elector records', async () => {
    await expect(
      electionSharedMutators.updateCandidate.fn({
        tx: createTx([null], 'server') as never,
        ctx,
        args: { id: 'missing', name: 'X' } as never,
      })
  ...
  it('validates linked indicative and final participation ownership on the server', async () => {
    ...
    await expect(
      electionSharedMutators.createIndicativeCandidateSelection.fn({
        tx: createTx([candidate, null], 'server') as never,
        ctx,
        args: selection,
      })
    ...
    await expect(
      electionSharedMutators.createIndicativeCandidateSelection.fn({
        tx: createTx([candidate, { ...participation, user_id: 'user-2' }], 'server') as never,
        ctx,
        args: selection,
      })
    ).rejects.toThrow('does not own');
    const indicative = createTx([candidate, { ...participation, user_id: ctx.userID }], 'server');
    await electionSharedMutators.createIndicativeCandidateSelection.fn({
      tx: indicative as never,
      ctx,
      args: selection,
    });
    ...
    await expect(
      electionSharedMutators.createFinalCandidateSelection.fn({
        tx: createTx([candidate, { ...participation, elector_id: null }], 'server') as never,
        ctx,
        args: selection,
      })
  ...
  it('covers manager and active-right fallbacks while ensuring elector rows', async () => {
    ...
    await expect(
      electionSharedMutators.replaceIndicativeElectionVote.fn({
        tx: noEvent as never,
        ctx,
        args: { elector, participation, selections: [] },
      })
    ).rejects.toBeInstanceOf(PermissionError);
  ...
  it('covers unlinked secret selection ownership and manager fallback', async () => {
    ...
    await electionSharedMutators.createIndicativeCandidateSelection.fn({
      tx: indicativeFallback as never,
      ctx,
      args: { ...selection, elector_participation_id: null },
    });
  ...
  it('covers client indicative replacement validation and named/secret persistence', async () => {
    ...
    const named = createTx([{ id: 'election-1', ballot_visibility: 'named' }, null]);
    await electionSharedMutators.replaceIndicativeElectionVote.fn({
      tx: named as never,
      ctx,
      args: { elector, participation, selections: [selection] },
    });
    ...
    const secret = createTx([{ id: 'election-1', ballot_visibility: 'secret' }, null]);
    await electionSharedMutators.replaceIndicativeElectionVote.fn({
      tx: secret as never,
      ctx,
    ...
    await expect(
      electionSharedMutators.replaceIndicativeElectionVote.fn({
        tx: createTx([{ id: 'election-1' }]) as never,
        ctx,
    ...
    await electionSharedMutators.replaceIndicativeElectionVote.fn({
      tx: repeat as never,
      ctx,
      args: { participation, selections: [selection] },
    });
