...
const agendaArgs = {
  id: 'agenda-1', event_id: 'event-1', amendment_id: null, title: null, description: null,
  type: 'discussion', status: 'planned', forwarding_status: null, order_index: 0, duration: 30,
  scheduled_time: null, start_time: null, end_time: null, activated_at: null,
  completed_at: null, majority_type: null, time_limit: null, voting_phase: null,
} as unknown as Parameters<typeof agendaSharedMutators.createAgendaItem.fn>[0]['args'];
const speakerArgs = {
  id: 'speaker-1', agenda_item_id: 'agenda-1', user_id: 'user-2', title: null,
  order_index: 0, time: null, completed: false, start_time: null, end_time: null,
...
describe('agenda shared mutator branch contracts', () => {
  it('handles client, event, amendment, missing-parent, and missing-item access', async () => {
    ...
    expect(canMock).toHaveBeenCalledWith(
      event, ctx, { action: 'create', resource: 'agendaItems', eventId: 'event-1' }
    );
    ...
    expect(canMock).toHaveBeenCalledWith(
      amendment, ctx, { action: 'update', resource: 'amendments', amendmentId: 'amendment-1' }
    );
    ...
    await expect(
      agendaSharedMutators.updateAgendaItem.fn({
        tx: createTx([null]) as never, ctx, args: { id: 'missing', title: 'Missing' },
      })
  ...
  it('repairs only eligible roles without an existing speak right', async () => {
    const tx = createTx([
      ...
      [
        ...
        {
          id: 'participant', name: 'Participant', assignee_kind: 'participant',
          action_rights: [
            { resource: 'events', action: 'view' }, { resource: 'groups', action: 'speak' },
          ],
        },
        {
          id: 'organizer', name: 'Organizer', assignee_kind: 'participant',
  ...
  it('keeps client speaker adds optimistic and rejects missing-event and unexpected permission errors', async () => {
    ...
    await expect(
      agendaSharedMutators.addSpeaker.fn({
        tx: unexpected as never, ctx, args: { ...speakerArgs, user_id: ctx.userID },
      })
  ...
  it('applies the gender quota when the speaker user record is missing', async () => {
    const tx = createTx([
      { id: 'agenda-1', event_id: 'event-1' }, [],
      { id: 'event-1', gender_quota_enabled: true }, null, [],
    ]);
    await expect(
      agendaSharedMutators.addSpeaker.fn({
        tx: tx as never, ctx, args: { ...speakerArgs, user_id: ctx.userID },
      })
    ).rejects.toThrow();
  });
  ...
  it('covers speaker removal and update authorization paths', async () => {
    ...
    await agendaSharedMutators.removeSpeaker.fn({
      tx: client as never, ctx, args: { id: 'speaker-1' },
    });
    await agendaSharedMutators.updateSpeaker.fn({
      tx: client as never, ctx, args: { id: 'speaker-1', completed: true },
    });

    await expect(
      agendaSharedMutators.removeSpeaker.fn({
        tx: createTx([null]) as never, ctx, args: { id: 'missing' },
      })
    ...
    await agendaSharedMutators.removeSpeaker.fn({
      tx: self as never, ctx, args: { id: 'speaker-1' },
    });
    ...
    await agendaSharedMutators.updateSpeaker.fn({
      tx: update as never, ctx, args: { id: 'speaker-1', completed: true },
    });
    expect(canMock).toHaveBeenCalledWith(
      update, ctx, { action: 'manage_speakers', resource: 'events', eventId: 'event-1' }
    );
    ...
    await agendaSharedMutators.updateSpeaker.fn({
      tx: amendmentUpdate as never, ctx, args: { id: 'speaker-1', completed: true },
    });
    expect(canMock).toHaveBeenCalledWith(
      amendmentUpdate,
      ctx,
      { action: 'update', resource: 'amendments', amendmentId: 'amendment-1' }
    );
    ...
    await agendaSharedMutators.removeSpeaker.fn({
      tx: other as never, ctx, args: { id: 'speaker-1' },
    });
    expect(canMock).toHaveBeenCalledWith(
      other, ctx, { action: 'manage_speakers', resource: 'events', eventId: 'event-1' }
    );
  });
  ...
  it('executes the remaining agenda and server-override facades', async () => {
    ...
    await agendaSharedMutators.createFull.fn({
      tx: client as never, ctx, args: { agenda_items: [agendaArgs], roles: [], elections: [], votes: [] },
    });
    await agendaSharedMutators.updateAgendaItem.fn({
      tx: client as never, ctx, args: { id: 'agenda-1', title: 'Updated' },
    });
    await agendaSharedMutators.reorderAgendaItems.fn({
      tx: client as never, ctx, args: { items: [{ id: 'agenda-1', order_index: 2 }] },
    });
    await agendaSharedMutators.deleteAgendaItem.fn({
      tx: client as never, ctx, args: { id: 'agenda-1' },
    });
    await agendaSharedMutators.createAgendaItemChangeRequest.fn({
      tx: client as never,
      ctx,
      args: {
        id: 'link-1', agenda_item_id: 'agenda-1', change_request_id: null, vote_id: null,
        order_index: 0, step_kind: 'closing', process_branch_id: null,
        is_closing_vote: true, status: 'pending', blocked_reason: null,
        result_status: null, obsolete_reason: null,
      },
    });
    await agendaSharedMutators.initializeChangeRequestVoting.fn({
      tx: client as never, ctx,
  ...
  it('covers client and server change-request row guards', async () => {
    ...
    await agendaSharedMutators.updateAgendaItemChangeRequest.fn({
      tx: client as never, ctx, args: { id: 'link-1', status: 'completed' },
    });
    await agendaSharedMutators.reorderAgendaItemChangeRequests.fn({
      tx: client as never, ctx, args: { items: [{ id: 'link-1', order_index: 1 }] },
    });
    await agendaSharedMutators.deleteAgendaItemChangeRequest.fn({
      tx: client as never, ctx, args: { id: 'link-1' },
    });

    await expect(
      agendaSharedMutators.updateAgendaItemChangeRequest.fn({
        tx: createTx([null]) as never, ctx, args: { id: 'missing', status: 'completed' },
      })
    ).rejects.toThrow('not found');
    await expect(
      agendaSharedMutators.reorderAgendaItemChangeRequests.fn({
        tx: createTx([null]) as never, ctx,
    ...
    for (const [name, invoke] of [
      ['update', (tx: ReturnType<typeof createTx>) =>
        agendaSharedMutators.updateAgendaItemChangeRequest.fn({
          tx: tx as never, ctx, args: { id: 'link-1', status: 'completed' },
        })],
      ['reorder', (tx: ReturnType<typeof createTx>) =>
        agendaSharedMutators.reorderAgendaItemChangeRequests.fn({
          tx: tx as never, ctx, args: { items: [{ id: 'link-1', order_index: 1 }] },
        })],
      ['delete', (tx: ReturnType<typeof createTx>) =>
        agendaSharedMutators.deleteAgendaItemChangeRequest.fn({
          tx: tx as never, ctx, args: { id: 'link-1' },
        })],
    ] as const) {
