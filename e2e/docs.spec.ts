...
test.describe('public documentation', () => {
  test('searches sections by keyboard and opens their anchors @smoke', async ({ page }) => {
    ...
    const result = page.getByRole('option', { name: /Votes|Abstimmungen/ }).first();
  ...
  test('renders the article navigation and redirects legacy links @smoke', async ({ page }) => {
  ...
  test('uses a drawer and collapsible page overview on mobile @smoke', async ({ page }) => {
